...
## 背景  
PostgreSQL的功能非常强大，但是要把PostgreSQL用好是非常关键的，下面的开发规范是针对PostgreSQL数据库的，这些强制和建议都是非常有帮助的，希望可以减少大家在使用数据库过程中遇到的困惑。  
  
将PostgreSQL的功能、性能发挥好。  
...
(2).比如A表user_name字段的校对规则是utf8_bin大小写敏感，但B表user_user字段的校对规则是utf8_general_ci大小写不敏感，A表和B根据user_name join会导致隐士转换；  
...
【推荐】TRUNCATE TABLE 在功能上与不带 WHERE 子句的 DELETE 语句相同：二者均删除表中的全部行。但 TRUNCATE TABLE 比 DELETE 速度快，且使用的系统和事务日志资源少，但TRUNCATE无事务且不触发trigger，有可能造成事故，故不建议在开发代码中使用DDL语句。    
...
【推荐】大批量数据入库的优化，如果有大批量的数据入库，建议使用copy语法，或者 insert into table values (),(),...(); 的方式。    
提高写入速度。    
jdbc, libpq都提供了copy, insert批量插入的接口。  
...
select *,total_users-coalesce(lag(total_users,1) over (order by rn),0) AS new_users from (  
...
【推荐】对RT要求高的业务，请使用SLB链路或PROXY透传模式连接数据库。  
