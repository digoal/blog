## PostgreSQL 9.6 检查点SYNC_FILE_RANGE_WRITE改进 与 单机多实例环境下的问题浅析
...
2\. 禁止分配buffer时调用同步writeback(建议修改为异步)    
bgwriter也应该使用异步的write模式。  
本文未完全体现。  
```
...
3\. 检查点过程中如果触发evict或分配buffer的情况，使用同步模式的sync file range.  
