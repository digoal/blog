## PostgreSQL 9.6 检查点SYNC_FILE_RANGE 在单机多实例下的IO Hang问题浅析与优化
...
49799 dege.zzz  20   0 1300m 155m 155m S  0.0  0.0   0:00.59 postgres -B 1GB -c port=1922 -c listen_addresses=0.0.0.0 -c synchronous_commit=on -c full_page_writes=on -c wal_buffers=128MB -c wal_writer_flush_after=0 -c bgwriter_delay=10ms
49844 dege.zzz  20   0 1300m 129m 128m S  0.0  0.0   0:09.01 postgres: wal writer process                                                                                                                                                    
49845 dege.zzz  20   0 1300m 1952 1224 S  0.0  0.0   0:05.71 postgres: autovacuum launcher process                                                                                                                                           
49838 dege.zzz  20   0  113m  892  460 S  0.0  0.0   0:00.03 postgres: logger process                                                                                                                                                        
16531 dege.zzz  20   0 1300m 1.1g 1.1g D  0.0  0.2   1:22.71 postgres: postgres postgres 127.0.0.1(49777) INSERT                                                                                                                             
16534 dege.zzz  20   0 1300m 1.1g 1.1g D  0.0  0.2   1:22.32 postgres: postgres postgres 127.0.0.1(49778) INSERT                                                                                                                             
16535 dege.zzz  20   0 1300m 1.1g 1.1g D  0.0  0.2   1:22.73 postgres: postgres postgres 127.0.0.1(49780) INSERT                                                                                                                             
16537 dege.zzz  20   0 1300m 1.1g 1.1g D  0.0  0.2   1:22.43 postgres: postgres postgres 127.0.0.1(49781) INSERT                                                                                                                             
49842 dege.zzz  20   0 1301m 1.0g 1.0g D  0.0  0.2   0:23.70 postgres: checkpointer process                                                                                                                                                  
49846 dege.zzz  20   0  115m 1048  552 D  0.0  0.0   0:12.83 postgres: stats collector process                                                                                                                                               
49843 dege.zzz  20   0 1300m 978m 977m D  0.0  0.2   0:46.35 postgres: writer process
```
...
在检查点过程中，bgwriter或backend process从shared buffer产生的脏页write out时，需要调用write和sync file range ()  
...
2\. 解决不安定因素2 - 检查点最后的阶段，调用fsync前，确保fd的所有dirty page都已经write out。    
  
修改 pg_flush_data，使用以下flag  
```
       SYNC_FILE_RANGE_WAIT_BEFORE | SYNC_FILE_RANGE_WRITE
              Ensures that all pages in the specified range which were dirty when sync_file_range() was called are placed under write-out.  This is a start-write-for-data-integrity operation.
...
3\. 从OS内核层面解决IO hang的问题。  
  
## 代码优化举例  
1\. 修改writeback调用，使用同步模式，非异步模式。    
  
确保fsync时没有或只有少量的未落盘dirty page。  
```
$ vi src/backend/storage/file/fd.c
...
2\. BufferAlloc使用异步write   
这里直接关掉writeback调用，方便测试     
```
                                // ScheduleBufferTagForWriteback(&BackendWritebackContext,
                               //                                                                 &buf->tag);
```
   
3\. 检查点过程中如果触发evict或分配buffer的情况，使用同步模式的sync file range，涉及bgwriter 的代码 .    
