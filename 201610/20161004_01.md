## PostgreSQL 9.6 单元化 与 sharding (based on postgres_fdw) - 内核层支持前传
...
9.6针对postgres_fdw（即PostgreSQL外部数据源）再次增强，开始支持对sort, where, join的下推，支持remote cancel query, 与本地事务共存亡的设计实现全局一致性，从而使用FDW可以完美的实现数据库的sharding，单元化需求。      
## 一、FDW架构
架构如图  
