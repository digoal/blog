...
9.6针对postgres_fdw（即PostgreSQL外部数据源）再次增强，开始支持对sort, where, join的下推，支持remote cancel query, 与本地事务共存亡的设计, 可以很方便的实现跨库事务的一致性, 用户使用FDW可以对应用透明的实现数据库的sharding，单元化需求。      
...
5\. 支持master(coordinator)节点水平扩展  
6\. 支持segment(datanode)节点水平扩展  
7\. 支持函数和存储过程    
...
目前postgres_fdw支持本地+远程的一致性事务，JOIN\SORT\WHERE\DML的下推。  
  
2\. 结合PostgreSQL的表继承，用户可以使用postgres_fdw实现数据库的sharding，高效的解决了同数据源的DML，排序，JOIN； 同时还解决了跨库事务一致性问题。   
