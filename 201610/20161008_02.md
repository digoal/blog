...
但是并不是所有的QUERY使用索引排序就一定高效，例如带过滤条件的QUERY，过滤完之后再根据某些字段或表达式排序。  
  
这种QUERY的排序不一定能用上索引。  
  
当需要排序时，PostgreSQL数据库怎么处理的呢？   
  
通常需要考虑:  排序的数据量, work_mem的大小, 排序的算法。  
  
本文将针对数据量较大，无法在内存中完成排序时，PG9.6的改进(external sort改进)。  
...
通常不需要修改replacement_sort_tuples的值  
