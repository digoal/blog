...
### 延迟分析
使用pg_xlogdump分析被堵塞的redo rec的内容  
```
...
## 小结
create 和 drop schema的文件操作很多，是一个个文件进行的，而且都会记录在REDO中，如果schema中有很对对象并且有很多文件的话，会非常慢。  
        
create 和 drop database产生的日志少，系统调用也更少。   
      
schema不建议作为daas的模板环境频繁使用，建议使用database作为模板。  
