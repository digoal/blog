...
rowid int表示，也就是说单个APPID不能允许超过20亿的用户。    
...
begin
  loop 
	...
	continue;
      ...
      if pg_try_advisory_xact_lock(1 * i_appid::int8) then
	insert into t_userid_dic (appid, rowid, userid) values (i_appid, 1, i_userid);
        return 1;
      else
	continue;
      end if;
...
(
  ( 
    ...
    (
      select t1.userids & t2.userids & t3.userids & t4.userids  -- & ......
       from
       ( -- 根据最大长度, BIT补齐 
