...
目前pg_pathman没有考虑prepared statement过滤分区，所以这里使用了simple query  
...
7\. 目前prepared statement不支持过滤分区，已发ISSUE，将来可能会解决。  
  
8\. 传统哈希分区需要输入分区键值的约束条件，才能正确选择分区。pg_pathman只要输入键值即可。  
