...
去年GP开源后，阿里云RDS PostgreSQL的小伙伴将Greenplum的分区表功能port到PostgreSQL 9.4之后，比使用继承和触发器的方式性能提升了近百倍，由于需要改动CATALOG，所以一直没有在9.4的版本上线这个功能。  
...
alter table test_pg_part_orig_1 add constraint ck_test_pg_part_orig_1 check(id >=1 and id<100000);
alter table test_pg_part_orig_2 add constraint ck_test_pg_part_orig_2 check(id >=100000 and id<200000);
alter table test_pg_part_orig_3 add constraint ck_test_pg_part_orig_3 check(id >=200000 and id<300000);
alter table test_pg_part_orig_4 add constraint ck_test_pg_part_orig_4 check(id >=300000 and id<400000);
alter table test_pg_part_orig_5 add constraint ck_test_pg_part_orig_5 check(id >=400000 and id<500000);
alter table test_pg_part_orig_6 add constraint ck_test_pg_part_orig_6 check(id >=500000 and id<600000);
alter table test_pg_part_orig_7 add constraint ck_test_pg_part_orig_7 check(id >=600000 and id<700000);
alter table test_pg_part_orig_8 add constraint ck_test_pg_part_orig_8 check(id >=700000 and id<800000);
alter table test_pg_part_orig_9 add constraint ck_test_pg_part_orig_9 check(id >=800000 and id<900000);
alter table test_pg_part_orig_10 add constraint ck_test_pg_part_orig_10 check(id >=900000 and id<1000000);

create or replace function tg_ins() returns trigger as $$
declare
  id int := NEW.id;
begin
  if NEW.id >=1 and NEW.id<100000 then
  ...
  elsif NEW.id >=500000 and NEW.id<600000 then
    insert into test_pg_part_orig_6 values (NEW.*);
  elsif NEW.id >=600000 and NEW.id<700000 then
    insert into test_pg_part_orig_7 values (NEW.*);
  elsif NEW.id >=700000 and NEW.id<800000 then
    insert into test_pg_part_orig_8 values (NEW.*);
  elsif NEW.id >=800000 and NEW.id<900000 then
    insert into test_pg_part_orig_9 values (NEW.*);
  elsif NEW.id >=900000 and NEW.id<1000000 then
...
Check constraints:
    "pathman_test_pg_part_pathman_1_1_check" CHECK (id >= 1 AND id < 100001)
...
Check constraints:
    "pathman_test_pg_part_pathman_10_1_check" CHECK (id >= 900001 AND id < 1000001)
Inherits: test_pg_part_pathman
```
...
postgres=# \timing
Timing is on.
postgres=# insert into test_pg_part_orig select generate_series(2,999999);
INSERT 0 0
Time: 40926.052 ms
...
```
postgres=# insert into test_pg_part_pathman select generate_series(2,999999);
INSERT 0 999998
Time: 5627.162 ms
postgres=# select count(*) from test_pg_part_orig;
 count  
...
\set id random(1,999999)
select * from test_pg_part_orig where id=:id;

$ pgbench -M prepared -n -r -P 1 -f ./test1.sql -c 32 -j 32 -T 120
tps = 74257.348446 (including connections establishing)
tps = 74262.492873 (excluding connections establishing)
```
...
tps = 163774.178165 (including connections establishing)
tps = 163781.956510 (excluding connections establishing)
...
$ pgbench -M prepared -n -r -P 1 -f ./test1.sql -c 32 -j 32 -T 120
tps = 47130.211557 (including connections establishing)
tps = 47132.981812 (excluding connections establishing)
```
...
$ pgbench -M prepared -n -r -P 1 -f ./test2.sql -c 32 -j 32 -T 120
tps = 49830.313578 (including connections establishing)
tps = 49832.863844 (excluding connections establishing)
```
...
postgres=# insert into test_pg_part_single select generate_series(1,999999);
INSERT 0 999999
Time: 4321.806 ms
...
$ pgbench -M prepared -n -r -P 1 -f ./test3.sql -c 32 -j 32 -T 120
tps = 199406.885626 (including connections establishing)
tps = 199417.187947 (excluding connections establishing)
...
$ pgbench -M prepared -n -r -P 1 -f ./test3.sql -c 32 -j 32 -T 120
tps = 115410.203626 (including connections establishing)
tps = 115415.974109 (excluding connections establishing)
```
    
性能测试数据对比如图  
    
![pic2](20161025_01_pic_002.png)    
