...
### 标签                                                              
PostgreSQL , 数组 , 相似度 , 文本分析 , 图像分析 , 字符串分析                                                                                                                         
...
本文提到的技术实际上是很早以前的相似度计算的技术（当时还没有引入GiST索引），但是比较简单，容易理解，还是挺有意思的。     
      
## 从最简单的说起 - 如何计算两个数组的相似度  
假设有两个数组，里面分别有一些元素，这些元素假设是用来表示用户的画像的。      
     
那么通过计算不同用户的数组的相似度，就可以知道他们是否有共同的癖好，有没有话题可聊。     
...
前面分析了一同数组的相似度运算，马上会问了，字符串怎么搞，图片，或者其他的特殊类型怎么搞？     
