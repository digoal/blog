...
对店铺、商品ID进行改造（字典化），不能使用原始的店铺ID或商品ID，（所有相邻ID间隔一个固定值）例如店铺ID1与店铺ID2之间必须保持固定的范围(这个固定值必须大于范围表描述的最大枚举区间,比如100,就像100个档位)，才能根据s1,s2,...推算出来哪家店或商品，以及对应的OFFSET。     
  
比如某用户1逛了店铺id1=1店铺id2=101, 逛了100次（假设s1 10-1000对应范围表枚举ID=3）, 那么s1数组为([4, 104]),表示这个用户在某天逛了1,101号店铺，分别逛了10-1000次。
   
使用int如何包含店铺ID与RANGE值呢？   
  
比如我们在某个维度上设置了20个RANGE(枚举)值，那么原有(如商品)ID*20+range OFFSET即可得到需要存储在数组中的值。   
