## 恭迎万亿级实时营销(圈人)进入毫秒时代 - 万亿user_tags级推荐系统数据库设计   
...
1\. 精准，指对用户的描述精准，通常需要基于大量的用户行为数据，经历深度学习后形成的用户画像，或称之为标签系统。 标签的准确性关系到推荐的精准度，比如你可能不会对一个正常的年轻人推荐老花眼镜（当然如果有其他购买意向的标签来指出他有购买老化眼镜的欲望除外）。     
...
如果能做到以上三点，这样的广告平台就具备一定的竞争力了。     
       
除此之外还需要关注的是平台的成本，包括硬件的成本，开发成本，维护成本等。     
     
本文将以电商的推荐系统为例，介绍一下推荐系统的数据库设计与优化技巧。     
