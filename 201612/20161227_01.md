...
begin  
  ...
  loop   
    ...
    if cv_rfid is null then   
      select array_agg(unnest(rfid)) into cv_rfid from rfid_cv where loc::text=v_loc::text and sid between NEW.sid-interval '1 min' and NEW.sid+interval '1 min';  
    end if;   
