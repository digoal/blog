## 收敛查询优化 select (300 ids) not in (select ids from 300万)           
...
另外有一张日志表B，每条记录中的ID是来自前面那张小表的，但不是每个ID都出现在这张日志表中，比如说一天可能只有几十个ID会出现在这个日志表的当天的数据中（比如说一天下来，某个片区巡逻车辆被摄像头识别到的ID）。  
  
那么我怎么快速的找出今天没有出现的ID呢（哪些巡逻车辆没有出现在这个片区，是不是偷懒了？）？   
...
采用收敛查询优化后，耗时从 618794毫秒 降低到了 11毫秒 ，感觉一下子节约了好多青春。    
