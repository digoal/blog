...
在本文的案例中，以overlap相似算法为例。  
  
GIN索引会构建一个商品ID为KEY的树，VALUE是行号组成的 posting list或者tree，在来了一篇新的导购文章时，根据导购文章涉及的商品ID，首先在树中进行搜索，每命中一个KEY则CNT++1，当CNT小于设置的smlar.threshold overlap阈值时，说明不满足条件，直接返回空结果。  
  
当大于等于设置的阈值时，进行下一层的搜索，可能的方法  
  
1\. (对第一个KEY展开，拿到第一个CTID，然后根据这个CTID到其他KEY的posting TREE进行搜索，每命中一次CNT++1)。  
  
2\. 通用的bitmap index scan阶段，取出CTID对应的HEAP PAGE ID，每个KEY生成一个bitmap(每个bit位对应堆表的一页)，所有KEY的bitmap进行一次bitmapAnd操作，得到一个新的bitmap，然后进入bitmap heap scan状态，即到对应的PAGE取记录，然后根据SQL提供的索引查询条件 用CPU去recheck。  
