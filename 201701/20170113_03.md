...
## 背景   
微信小程序最近比较火，大概功能是以后大家都不需要安装程序了，直接在微信中运行即可。跑在微信里的小程序类似一个虚拟机或者一个Docker容器，这是腾讯要分APP store蛋糕的节奏吗？  
   
从技术角度来看，微信小程序带来了几个好处，不需要安装软件，用户不需要进行软件的版本管理，节省了一点点手机的空间，同时还减少了流氓软件的可能，确实还不错对吧。     
