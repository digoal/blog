...
## 小结  
这个问题可能是PostgreSQL尾部块回收的一个BUG，也可能是Linux ext4文件系统使用data=writeback mount参数，操作系统或文件系统CRASH后导致的问题。与社区沟通中。  
  
目前，如果您遇到读某个块只能读出0字节，可能和文中是同一个CASE，可以使用文中提到的QUERY，找到对应的对象名，如果对象是表或者物化视图，可以使用 ```vacuum analyze 名字;``` 来进行修复。  
