...
## 背景      
社区中某企业有去O的需求，他们自己调研了一下MySQL存在的弊端，心存疑虑，罗列如下。毫不犹豫的推荐他们使用PostgreSQL。  
  
因为这些弊端在PostgreSQL中都不存在，而且PostgreSQL多了很多更有的特性。  
...
#### 15. 执行计划并不是全局共享的,仅仅在连接内部是共享的；  
  
PostgreSQL 支持绑定变量，执行计划缓存。但是执行计划是会话级别，好处是同一个SQL被高并发调用时，没有Oracle shared_pin的问题。  
...
[《数据库选型 - 致 架构师、开发者》](../201702/20170209_01.md)  
