...
查询某一分钟(60条)、某2个小时(7200条)的数据，耗时47毫秒。  
  
```  
postgres=# explain analyze select * from tbl_sec_000001 where crt_time between '2017-04-17 14:21:00' and '2017-04-17 16:20:59';  
                                                                         QUERY PLAN                                                                            
-------------------------------------------------------------------------------------------------------------------------------------------------------------  
 Bitmap Heap Scan on tbl_sec_000001  (cost=834.57..7710.56 rows=5578 width=88) (actual time=45.897..47.132 rows=7200 loops=1)  
   Recheck Cond: ((crt_time >= '2017-04-17 14:21:00'::timestamp without time zone) AND (crt_time <= '2017-04-17 16:20:59'::timestamp without time zone))  
   Rows Removed by Index Recheck: 80  
   Heap Blocks: lossy=104  
   ->  Bitmap Index Scan on tbl_sec_000001_crt_time_idx  (cost=0.00..833.18 rows=5578 width=0) (actual time=45.885..45.885 rows=1040 loops=1)  
         Index Cond: ((crt_time >= '2017-04-17 14:21:00'::timestamp without time zone) AND (crt_time <= '2017-04-17 16:20:59'::timestamp without time zone))  
 Planning time: 0.048 ms  
 Execution time: 47.501 ms  
(8 rows)  
  
postgres=# explain analyze select * from tbl_sec_000001 where crt_time between '2017-04-17 14:21:00' and '2017-04-17 14:21:59';  
                                                                         QUERY PLAN                                                                            
-------------------------------------------------------------------------------------------------------------------------------------------------------------  
 Bitmap Heap Scan on tbl_sec_000001  (cost=777.40..778.72 rows=1 width=88) (actual time=46.477..46.492 rows=60 loops=1)  
   Recheck Cond: ((crt_time >= '2017-04-17 14:21:00'::timestamp without time zone) AND (crt_time <= '2017-04-17 14:21:59'::timestamp without time zone))  
   Rows Removed by Index Recheck: 80  
   Heap Blocks: lossy=2  
   ->  Bitmap Index Scan on tbl_sec_000001_crt_time_idx  (cost=0.00..777.40 rows=1 width=0) (actual time=46.460..46.460 rows=20 loops=1)  
         Index Cond: ((crt_time >= '2017-04-17 14:21:00'::timestamp without time zone) AND (crt_time <= '2017-04-17 14:21:59'::timestamp without time zone))  
 Planning time: 0.077 ms  
 Execution time: 46.527 ms  
(8 rows)  
```  
