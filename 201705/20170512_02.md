...
函数，判断是否已关注，如果已关注，返回异常，否则关注。(这个也可以写在程序中，但是需要与数据库交互多次，差评)  
...
declare  
begin  
  perform 1 from cv_user_like where uid=i_uid and like_who @> array[i_like_who];  
  if not found then  
...
### 首先根据以上要求生成基础数据  
压测脚本，LIKE文章，100万热点文章  
  
```  
vi test.sql  
\setrandom uid 1 100000000  
\setrandom hot_id 1 1000000  
\setrandom id 1 200000000  
select f_obj(:id,:uid);  
select f_obj(:hot_id,:uid);  
```  
  
256个连接进行压测，测试结果，相当于每秒产生17.4万次LIKE请求。   
...
duration: 120 s  
number of transactions actually processed: 10559022  
latency average: 2.908 ms  
latency stddev: 8.666 ms  
tps = 87858.834653 (including connections establishing)  
tps = 87864.175071 (excluding connections establishing)  
statement latencies in milliseconds:  
        0.002289        \setrandom uid 1 100000000  
        0.000656        \setrandom hot_id 1 1000000  
        0.000602        \setrandom id 1 200000000  
        1.437119        select f_obj(:id,:uid);  
        1.465519        select f_obj(:hot_id,:uid);  
```  
...
pipeline=# select * from cv_obj limit 10;  
    id     |                                     who_like                                     | like_cnt   
-----------+----------------------------------------------------------------------------------+----------  
   2098265 | {7306608,54824445,20424936,49757740,44084582,3550870,50276673,37109183,48132495} |        9  
 172248989 | {52639193} |        1  
 150584128 | {61082705} |        1  
 143947883 | {21691112} |        1  
 143437872 | {33829711} |        1  
 177665275 | {50700001} |        1  
 158870749 | {48700123} |        1  
  38330766 | {84921836} |        1  
 127562112 | {93179163} |        1  
 128510873 | {80591316} |        1  
```  
