## 快递员职业将消失？谈一谈无人驾驶、配送机器人背后的技术 - PostGIS支持点云(pointcloud)应用  
...
### 标签                             
PostgreSQL , PostGIS , box , grid , pointcloud , pgpointcloud , point聚合 , KNN , 自动驾驶 , 自动配送 , 无人驾驶      
                                  
----                            
                                     
## 背景       
科幻电影的场景随着技术的发展，正在一步步的从荧幕变成现实。比如汽车厂商、科技公司在尝试的无人驾驶。  
  
无人驾驶应用是非常广泛的，比如快递行业，时机成熟以后，将来可能快递员这个职业也会逐渐从社会上消失(解放快递员的双手和创造力，让更多的人参与到科技改变生活当中)。  
  
无人驾驶，少不了导航，导航少不了位置信息，位置信息又是由无数的点组成的。点越多，精细度越高，就像照片一样，以前的手机像素和感光元件很粗糙，拍摄的照片清晰度就不如现在的手机。  
...
为了方便描述，我们把point点阵称为点云(pointcloud)。当设定了无人驾驶的起始点和路径，需要将涉及道路的点都载入汽车，每个点除了位置，还应该有其他属性，比如窨井盖、坑、水泥路OR沥青路、红绿灯、硬路肩等，这些属性可以使用其他字段存储。  
...
在优化代码前，获取20万个点，可以1秒内返回。返回更大数据量时，可以通过pgpointcloud、网格化聚合等优化手段来提速，返回500万个点，仅需312毫秒。  
  
但是性能优化永远是没有止境的话题，加油小伙伴们，加油PostgreSQL，PostGIS, pgpointcloud。  
