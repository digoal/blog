...
由于属性取值空间可能连续，使用[《万亿级营销(圈人)潇洒的迈入毫秒时代 - 万亿user_tags级实时推荐系统数据库设计》](../201612/20161225_01.md)提到的方法，需要建立标签库，将数据阶梯化，查询也要进行转换。  
...
创建其他列的btree索引，因为其他列基本唯一，所以我们建立btree索引，可以精准的进行定位。  
