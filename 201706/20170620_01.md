...
因此对于一个这样的与PostGIS相关的QUERY，优化如下  
  
```  
explain (analyze,verbose,timing,costs,buffers)   
select xxx1,xxx2,xxx3,st_asbinary(geo) as geo,  
  ST_Transform (geo, 26986) ST_Transform (ST_GeomFromText ('POINT(121.403833486783 31.1425794813889)', 4326), 26986) as distance2Center  
  from tbl  
  ...
  order by ST_Transform (geo, 26986) ST_Transform (ST_GeomFromText ('POINT(121.403833486783 31.1425794813889)', 4326), 26986) ;  
...
在扫描时，自动检索对应的索引分区。达到分区表+独立索引同样的效果。  
