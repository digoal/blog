...
拜占庭将军的问题和三节点的问题一致，Raft成为了RDS解决三节点一致性、可靠性问题的普世算法。有了强大的理论基础，RDS 金融数据库(三节点版)的一致性、可靠性自然是有保障的。  
...
因此4.2如果没有发送成功，并且此时如果Leader异常，并发生了重新选举，有没有风险？New Leader的这笔事务到底是commit还是uncommit? (个人认为应该是commit，否则就违背了ACID的D持久化，就有问题了，就丢事务了。)     
...
这个阶段 Leader 挂掉，Cluster 内部数据其实已经是一致的，此时 Client 不知到底提交成功没有，也就是说客户端视角事务状态为UNKNOWN。      
    
![pic](20170709_01_pic_010.png)    
    
### 6. 数据到达 Leader 节点，成功复制到 Follower 所有或多数节点，数据在 Leader 处于已提交状态，但在 Follower 处于未提交状态。（实际上就是图中标注的4.2步骤没有被执行）    
