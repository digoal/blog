...
## 背景      
单身汉的房间可能是这样的，凌乱得很。（不是应该有充气WW吗？）。  
  
![pic](20170905_01_pic_003.jpg)  
  
![pic](20170905_01_pic_004.jpg)  
  
实际上数据大多数情况下也可能是这样的，索引的目的是在凌乱的数据中找到目标数据，但是从凌乱的数据中按索引顺序扫描一批数据，会有什么后果呢：  
...
为了测试方便，直接将GiST的叶子节点写到一张临时表。  
  
```  
postgres=# create table test1(pos point);  
CREATE TABLE  
  
insert into test1 select a::point from gist_print('idx_test_pos') as t(level int, valid bool, a box) where level =4;  
...
```  
postgres=# create index idx_test1 on test1 using gist(pos);  
```  
...
```  
explain (analyze,verbose,timing,costs,buffers) select * from test1 where box('(100,100),(200,200)') @> pos;  
  
                                                          QUERY PLAN                                                             
-------------------------------------------------------------------------------------------------------------------------------  
 Bitmap Heap Scan on public.test1  (cost=193.12..10549.51 rows=10000 width=16) (actual time=8.092..21.497 rows=101518 loops=1)  
   Output: pos  
   Recheck Cond: ('(200,200),(100,100)'::box @> test1.pos)  
   Heap Blocks: exact=691  
   Buffers: shared hit=1727  
   ->  Bitmap Index Scan on idx_test1  (cost=0.00..190.62 rows=10000 width=0) (actual time=7.997..7.997 rows=101518 loops=1)  
         Index Cond: ('(200,200),(100,100)'::box @> test1.pos)  
...
可以看到，使用空间聚集，某个空间查询条件返回101518记录，HEAP数据扫描从50914个数据块降低到了691个数据块。  
