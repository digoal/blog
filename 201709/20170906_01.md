...
```  
select dblink_send_query('外部表名_1','begin; insert into 外部表1 select * from tmp where mod(order_id,4)=0; update tbl_result set status='done' where task='外部表名_1'; end;');  
select dblink_send_query('外部表名_2','begin; insert into 外部表2 select * from tmp where mod(order_id,4)=1; update tbl_result set status='done' where task='外部表名_2'; end;');  
select dblink_send_query('外部表名_3','begin; insert into 外部表3 select * from tmp where mod(order_id,4)=2; update tbl_result set status='done' where task='外部表名_3'; end;');  
select dblink_send_query('外部表名_4','begin; insert into 外部表4 select * from tmp where mod(order_id,4)=3; update tbl_result set status='done' where task='外部表名_4'; end;');  
```  
...
通过查询tbl_result，如果status='done'的记录数不等于线程数，则说明有任务失败。  
