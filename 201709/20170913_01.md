...
#### 检查方法：  
1、主机级，输出每台主机的已使用空间（区分数据空间、临时空间、REDO空间），剩余空间（区分数据空间、临时空间、REDO空间）。

2、库级，以库为单位，输出每个库在每个segment的使用空间。
  
3、schema级，以schema为单位，输出每个schema在每个segment的使用空间。
  
4、用户级，以owner为单位，输出每个owner在每个segment的使用空间。  
  
5、表级，以表为单位，输出每个segment的使用空间。  
  
6、索引级，以索引为单位，输出每个segment的使用空间。    
...
#### 查询倾斜
除了数据分布倾斜，另一种倾斜是查询倾斜，通常更加查询计划可以发现查询倾斜，例如窗口查询，partition by会强制重分布，如果这个分组有倾斜，那么就可能出现查询倾斜。
...
写入、删除、更新记录数（SUM、库级、schema级、owner级、表级）。


容量变化 (表、索引、SCHEMA、DB级）。
