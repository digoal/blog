...
5、生成1年订单，约3.65亿（每天100万比订单，90%支付，10%退款）   
...
```  
date +%F%T; psql -c "copy (select did,state,crt_time,mod_time from log) to stdout"|psql -h 127.0.0.1 -p 15432 -U dege.zzz postgres -c "copy log1 from stdin"; date +%F%T;  
...
## 九、小结  
对于本例，建议还是使用PostgreSQL 10。性能方面，TP和AP都满足需求。功能方面也完全满足需求，而且有很多可以利用的特性来提升用户体验。  
  
将来阿里云会推出PolarDB for PostgreSQL，100TB级，共享存储，一写多读架构。对标AWSAurora与Oracle RAC。  
