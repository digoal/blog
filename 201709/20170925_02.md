...
```  
create table audit_ddl_command_end (  
  event text,  
  tag text,  
  username name,  
  datname name,  
...
create table audit_table_rewrite (  
  event text,  
  tag text,  
  username name,  
  datname name,  
...
grant select,update,delete,insert,truncate on audit_ddl_command_end to public;  
grant select,update,delete,insert,truncate on audit_sql_drop to public;  
grant select,update,delete,insert,truncate on audit_table_rewrite to public;  
```  
...
declare  
begin  
  insert into audit_ddl_command_end   
    select TG_EVENT, TG_TAG, current_user, current_database(), inet_client_addr(), inet_client_port(), now(),   
...
```  
create or replace function et_table_rewrite() returns event_trigger as $$  
declare  
begin  
  insert into audit_table_rewrite   
    select TG_EVENT, TG_TAG, current_user, current_database(), inet_client_addr(), inet_client_port(), now(),   
...
CREATE EVENT TRIGGER et_table_rewrite on table_rewrite EXECUTE PROCEDURE et_table_rewrite();  
```  
...
```  
db1=> select * from audit_ddl_command_end ;  
-[ RECORD 1 ]---+---------------------------  
...
```  
db1=> select * from audit_sql_drop ;  
-[ RECORD 1 ]---+--------------------------------  
event           | sql_drop  
tag             | DROP TABLE  
username        | test  
...
```  
db1=> select * from audit_table_rewrite ;  
...
create or replace function ef_ddl_command_end() returns event_trigger as $$  
declare  
  rec hstore;  
begin  
  select hstore(pg_stat_activity.*) into rec from pg_stat_activity where pid=pg_backend_pid();  
  insert into audit_ddl_command_end (event, tag, ctx) values (TG_EVENT, TG_TAG, rec);  
end;  
$$ language plpgsql strict;  

create event trigger ef_ddl_command_end on ddl_command_end execute procedure ef_ddl_command_end();  

create or replace function ef_sql_drop() returns event_trigger as $$  
declare  
begin  
  insert into audit_sql_drop (event, tag, classid, objid, objsubid, object_type, schema_name, object_name, object_identity)  
...
create event trigger ef_sql_drop on sql_drop execute procedure ef_sql_drop();  
```
