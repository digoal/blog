...
--------+------+-------+----------+-------+-------------  
 public | tbl  | table | postgres | 11 GB |   
(1 row)  
...
test=# explain (analyze,verbose,timing,costs,buffers) select * from tbl where c1 between 1 and 1000;  
                                                    QUERY PLAN                                                      
------------------------------------------------------------------------------------------------------------------  
 Bitmap Heap Scan on public.tbl  (cost=65.53..29931.60 rows=1 width=12) (actual time=2.562..2.562 rows=0 loops=1)  
   Output: c1, c2, c3  
   Recheck Cond: ((tbl.c1 >= 1) AND (tbl.c1 <= 1000))  
   Buffers: shared hit=65  
   ->  Bitmap Index Scan on idx  (cost=0.00..65.53 rows=23678 width=0) (actual time=2.559..2.559 rows=0 loops=1)  
...
test=# explain (analyze,verbose,timing,costs,buffers) select * from tbl where c2 between 1 and 1000;  
                                                    QUERY PLAN                                                      
------------------------------------------------------------------------------------------------------------------  
 Bitmap Heap Scan on public.tbl  (cost=65.53..29931.60 rows=1 width=12) (actual time=2.582..2.582 rows=0 loops=1)  
   Output: c1, c2, c3  
   Recheck Cond: ((tbl.c2 >= 1) AND (tbl.c2 <= 1000))  
   Buffers: shared hit=65  
   ->  Bitmap Index Scan on idx  (cost=0.00..65.53 rows=23678 width=0) (actual time=2.579..2.579 rows=0 loops=1)  
...
test=# explain (analyze,verbose,timing,costs,buffers) select * from tbl where c3 between 1 and 1000;  
                                                    QUERY PLAN                                                      
------------------------------------------------------------------------------------------------------------------  
 Bitmap Heap Scan on public.tbl  (cost=65.53..29931.60 rows=1 width=12) (actual time=2.374..2.374 rows=0 loops=1)  
   Output: c1, c2, c3  
   Recheck Cond: ((tbl.c3 >= 1) AND (tbl.c3 <= 1000))  
   Buffers: shared hit=65  
   ->  Bitmap Index Scan on idx  (cost=0.00..65.53 rows=23678 width=0) (actual time=2.373..2.373 rows=0 loops=1)  
...
test=# explain (analyze,verbose,timing,costs,buffers) select * from tbl where c1 between 1 and 1000 and c2 between 100 and 2000;  
                                                    QUERY PLAN                                                      
------------------------------------------------------------------------------------------------------------------  
 Bitmap Heap Scan on public.tbl  (cost=65.53..30049.99 rows=1 width=12) (actual time=2.451..2.451 rows=0 loops=1)  
   Output: c1, c2, c3  
   Recheck Cond: ((tbl.c1 >= 1) AND (tbl.c1 <= 1000) AND (tbl.c2 >= 100) AND (tbl.c2 <= 2000))  
   Buffers: shared hit=65  
   ->  Bitmap Index Scan on idx  (cost=0.00..65.53 rows=23678 width=0) (actual time=2.450..2.450 rows=0 loops=1)  
...
test=# explain (analyze,verbose,timing,costs,buffers) select * from tbl where c1 between 1 and 1000 and c3 between 100 and 2000;  
                                                    QUERY PLAN                                                      
------------------------------------------------------------------------------------------------------------------  
 Bitmap Heap Scan on public.tbl  (cost=65.53..30049.99 rows=1 width=12) (actual time=2.383..2.383 rows=0 loops=1)  
   Output: c1, c2, c3  
   Recheck Cond: ((tbl.c1 >= 1) AND (tbl.c1 <= 1000) AND (tbl.c3 >= 100) AND (tbl.c3 <= 2000))  
   Buffers: shared hit=65  
   ->  Bitmap Index Scan on idx  (cost=0.00..65.53 rows=23678 width=0) (actual time=2.382..2.382 rows=0 loops=1)  
...
test=# explain (analyze,verbose,timing,costs,buffers) select * from tbl where c1 between 1 and 1000 and c2 between 100 and 2000 and c3 between 1 and 2000;  
                                                                QUERY PLAN                                                                  
------------------------------------------------------------------------------------------------------------------------------------------  
 Bitmap Heap Scan on public.tbl  (cost=65.53..30168.38 rows=1 width=12) (actual time=2.392..2.392 rows=0 loops=1)  
   Output: c1, c2, c3  
   Recheck Cond: ((tbl.c1 >= 1) AND (tbl.c1 <= 1000) AND (tbl.c2 >= 100) AND (tbl.c2 <= 2000) AND (tbl.c3 >= 1) AND (tbl.c3 <= 2000))  
   Buffers: shared hit=65  
   ->  Bitmap Index Scan on idx  (cost=0.00..65.53 rows=23678 width=0) (actual time=2.391..2.391 rows=0 loops=1)  
