## PostgreSQL Oracle 兼容性 之 TABLE、PIPELINED、put_line函数(流式返回、返回多行、返回表、返回任意结构)  
...
postgres=# select * from demo('select oid::text::int from pg_class limit 10', 'select 2') as t(id int);
  id   
-------
 25041
 37447
