...
### 索引
#### 2. 当使用GIN索引，并且大量使用了LIMIT来限制输出时，建议使用rum索引方法。避免bitmap index scan的recheck耗时。    
根据场景自动选择gin\rum索引的需求。
    
### 性能
#### 3. PostgreSQL，全表扫描支持通过hint或开关来使用directio，不占用OS CACHE，支持不加载到SHARED BUFFER。    
目前大表扫描，超过四分之一SHARED BUFFER的表，会设置FLAG，分批读取，并优先刷出SHARED BUFFER，防止扫描大表时SHARED BUFFER的抖动。
    
### 功能
#### 4. PostgreSQL，每个DB有单独的REDO，DB支持热插拔。支持DB级的物理流复制。一个集群的数据库可以物理流复制的模式拷贝到另一个集群。   
    
Oracle 18c 已支持    
    
#### 9. postgresql , 并行写wal。目前极限写压力下，WAL会成为瓶颈。      
  
#### 5. PostgreSQL，update returning old.column      
      
#### 6. PostgreSQL，列存储，支持并行的列存储。    
    
#### 7. PostgreSQL， online ddl , 可以改进pg_repack来支持ONLINE DDL。     
    
#### 8. pgbench 支持动态对象名，例如    
...
#### 9. 类似GIT的PG函数语言的版本迭代控制
pg的存储过程接口非常丰富，用到的用户也非常多，版本控制的功能非常重要。
  
#### 10. postgresql , HTAP业务，资源队列管理，资源隔离，进程组管理.    
    
#### 11. grouping sets, rollup, cube, grouping id函数       
    
http://blog.csdn.net/huang_xw/article/details/6402396    
    
#### 12 grouping()、grouping_id()、group_id()    
...
### PostgreSQL 内核  
#### 25. postgresql, 更新合并，对应秒杀场景(库存扣减)。    
...
#### 41. 通过HINT 在dml中包含begin, end事务标记    
...
#### 42. update|delete skip locked, nowait语法支持    
...
### 安全
#### 53. 自动隐藏隐私信息（如create user password, alter role.... password)    
    
#### 54. sql 防火墙     
    
#### 55. 并行恢复、未达到一致性点之前，恢复过程允许只读操作，自动过滤不一致数据块，或自动使用旧快照。   快速打开库允许只读。  
    
#### 56. 在log_min_duration和 auto_explain记录的SQL中记录锁等待的时长    
    
#### 57. 使用copy导入数据时，跳过异常的行。    
    
#### 58. walsender支持restore_command取文件传送给walreceiver    
...
#### 62. pg_basebackup 过滤 hash index & unlogged table    
    
    
    
#### 63. 支持每个USER多个密码，每个密码有对应的过期时间    
...
#### 66. 资源隔离 : 会话级、用户级、语句级、库级 内存、CPU单位时间、IOPS  限制    
...
#### 73. 改进垃圾回收进程，只保留需要的tuple版本，而不是最早事务之前的所有版本。    
    
    
#### 74. 便捷的各种数据类产品打通，同步。    
数据同步模块  
    
#### 76. postgresql, pg_stat_all_tables, 建议添加autovacuum, auto analyze, analyze, vacuum等存储为数组，记录最近N次的操作统计信息，包括每次扫描了多少BLOCK，产生了多少DIRTY PAGE等等。    
...
#### 76. PG，GP， 数据采样（已支持）、脱敏功能。    
    
#### 77. 数据类型， 透明加密功能.     
    
#### 78. Greenplum, postgresql , 加入roaringbitmap，同时支持 多阶段并行聚合函数。    
    
#### 79. PostgreSQL , 在创建分区表索引时，支持每个分区并行创建。    
    
    
#### 80. PostgreSQL , paralle append + 外部表 + pushdown，可以实现sharding 架构下的并发计算。（例如求SUM,AVG,COUNG,MIN,MAX等，不过针对外部表的parallel append内核层面还没有支持好，需要CUSTOM SCAN）    
    
#### 81. PostgreSQL, 支持单个索引含多颗树 。     
    
[《PostgreSQL 店铺运营实践 - JSON[]数组 内部标签数据等值、范围检索100倍+加速示例》](../201802/20180208_01.md)      
    
分区索引。  
  
#### 82. 支持分区索引。一个索引根据某些表达式、字段HASH、范围分区，构成多颗树。（时间、空间、多个属性）    
    
#### 83. GIN索引支持范围扫描（目前仅支持等值(包含、相交等)扫描）。    
...
#### 84. PostgreSQL, GPDB , jsonbd 一种内置压缩能力的JSON类型，实际上数据库内核也可以在数组、全文检索等其他多值类型上增加类似的压缩功能（相当于内置的数据字典能力），将字典化这个工作转嫁给数据库来实现。       
...
#### 85. 支持index skip scan
目前通过cte递归来支持  
    
#### 85. online split, merge 分区表.    
    
#### 86. postgresql, gpdb 支持动态执行计划，执行过程中根据实际的NODE扫描并返回的数据的统计信息，动态调整执行计划。    
    
#### 87. timescale , postgresql, 提供数据自动老化能力（自动有损压缩）。    
      
#### 88. postgresql, 支持具备阶段性可靠性的UNLOGGED TABLE，加速导入。    
      
#### 89. postgresql , skip locked , 返回未获得锁的行。以便再次处理。    
      
#### 90. 支持冻结从库的receiver进程，不接受主库的wal。目前仅支持replay的冻结。     
...
#### 91. postgresql, 支持flashback, 例如postgresql 主备切换时，老的主库在没有配置recovery.conf时启动并有写入，导致分裂。使用flashback可以快速回到上一个时间线。    
    
使用pg_rewind可以实现同样功能。内部实现可以在启动后记录若干个变更的UNDO，从而实现可以回退。    
  
#### 支持普通用户设置synchronous_commit
