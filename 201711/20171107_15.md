...
生成200万随机字符串(总长度16的一些TOKEN)，用于作为全文检索的条件输入。    
    
```    
create table t_fts_test (id serial primary key, info text);    
    
insert into t_fts_test (info) select gen_rand_str(16) from generate_series(1,2000000);    
```    
...
duration: 300 s  
number of transactions actually processed: 1993160  
latency average = 8.431 ms  
latency stddev = 39.315 ms  
tps = 6620.032099 (including connections establishing)  
tps = 6620.838135 (excluding connections establishing)  
script statistics:  
 - statement latencies in milliseconds:  
         0.002  \set id random(1,2000000)    
         8.448  select * from get_t_fts_test(:id);  
```    
