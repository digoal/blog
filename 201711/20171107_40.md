...
方法|QPS
---|---
logged 多表写入-硬解析 | 170812
logged 多表写入-绑定变量 | 218507
unlogged 多表写入-绑定变量 | 221138
  
unlogged table 的高并发多表写入性能并没有大量提升，原因是commit也要写WAL日志，只是table以及对应的toast table, index, toast index的变更不记录日志了。所以unlogged table的高并发小事务写入，wal writer依旧是瓶颈。  
