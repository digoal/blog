...
\set x random(120,130)  
\set y random(70,80)  
select id from tbl_pos where att1=:att1 and att2=:att2 and ST_DistanceSpheroid(pos , st_setsrid(st_makepoint(:x, :y), 4326), 'SPHEROID["WGS84",6378137,298.257223563]') < 5000 order by pos <-> st_setsrid(st_makepoint(:x, :y), 4326) limit 100;  
```  
...
script statistics:  
 - statement latencies in milliseconds:  
         ...
         0.000  \set x random(120,130)  
         0.000  \set y random(70,80)  
         0.842  select id from tbl_pos where att1=:att1 and att2=:att2 and ST_DistanceSpheroid(pos , st_setsrid(st_makepoint(:x, :y), 4326), 'SPHEROID["WGS84",6378137,298.257223563]') < 5000 order by pos <-> st_setsrid(st_makepoint(:x, :y), 4326) limit 100; 
```    
