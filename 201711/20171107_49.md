## HTAP数据库 PostgreSQL 场景与性能测试之 3.1 - (OLAP) 大表JOIN统计查询       
...
```    
alter table t_user set (parallel_workers =32);  
alter table t_user_log_20191212 set (parallel_workers =32);  
...
postgres=# explain  select c1,count(*) from t_user join t_user_log_20191212 using (uid) group by c1;  
                                                    QUERY PLAN                                                      
------------------------------------------------------------------------------------------------------------------  
 Finalize HashAggregate  (cost=13710582.40..13710582.51 rows=11 width=12)  
   Group Key: t_user.c1  
   ->  Gather  (cost=13710580.75..13710580.86 rows=308 width=12)  
         Workers Planned: 28  
         ->  Partial HashAggregate  (cost=13710580.75..13710580.86 rows=11 width=12)  
               Group Key: t_user.c1  
               ->  Parallel Hash Join  (cost=1234160.17..13532009.31 rows=35714288 width=4)  
                     Hash Cond: (t_user_log_20191212.uid = t_user.uid)  
                     ->  Parallel Seq Scan on t_user_log_20191212  (cost=0.00..11720779.88 rows=35714288 width=8)  
                     ->  Parallel Hash  (cost=1172078.30..1172078.30 rows=3571430 width=12)  
                           ->  Parallel Seq Scan on t_user  (cost=0.00..1172078.30 rows=3571430 width=12)  
(11 rows)  
