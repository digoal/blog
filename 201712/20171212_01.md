...
loop  
  ...
  execute format('insert into tag_userbitmap_%s (tagid, usrebitmap) values (%s, set_bit_array(''0'', %s, %s, %L))   
                  on conflict (tagid)   
		  do update set usrebitmap=set_bit_array(tag_userbitmap_%s.usrebitmap, %s, %s, %L) where tag_userbitmap_%s.tagid=%s',   
		  v_suffix2, v_tagid, v_action, 0, v_fixed_dict_id, v_suffix2, v_action, 0, v_fixed_dict_id, v_suffix2, v_tagid);  
end loop;  
end;  
$$ language plpgsql strict;  
```  
  
调用，增量更新TAG，不同suffix组合，允许并行调用。  
