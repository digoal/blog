...
create index idx_tbl_1 on tbl using gist (sid, hid, ts);  
create index idx_tbl_5min_1 on tbl_5min using gist (sid, hid, ts);  
create index idx_tbl_30min_1 on tbl_5min using gist (sid, hid, ts);  

create index idx_tbl_2 on tbl using btree (sid, hid, ts);  
create index idx_tbl_5min_2 on tbl_5min using btree (sid, hid, ts);  
create index idx_tbl_30min_2 on tbl_5min using btree (sid, hid, ts);  
```  
...
create or replace function get_val(v_sid int, v_hid int, v_ts timestamp) returns tp as $$  
  select id, sid, hid, val, ts from tbl where sid=v_sid and hid=v_hid order by ts <-> v_ts limit 1;  
$$ language sql strict;  
```  
