...
如果不需要查权重, 则只需要创建c1字段的索引, 效率会更高一点点.

create index on a(c1);    
```      
      
2、生成测试数据：      
      
```    
这条sql 一次生成1个ID的500条关系. 

vi test.sql      
      
\set id random(1,10000000) 
insert into a select :id, ((width_bucket(:id,1,10000000,200)-1)*50000 + (random()*50000)::int) from generate_series(1,500) on conflict (c1,c2) do nothing;      
```      

id随机产生, 可能会有重复出现的情况, 也就是1-1000万的ID区间里面, 有些ID可能超过500条关系, 有些ID无记录.  
      
```  
50个并发, 每个执行200000次 
5000000000/500/50 = 200000  

pgbench -M prepared -n -r -P 5 -f ./test.sql -c 50 -j 50 -t 200000        
```      
