...
## 例子2 - 1亿 JOIN 1亿  
1、PostgreSQL 11，1亿 JOIN 1亿，通过条件过滤1000万。  
...
1亿(过滤1千万) JOIN 1亿(过滤1千万) | 6.35秒 | 1.24秒  
1亿 JOIN 1亿(过滤1千万) | 8.5秒 | 2秒  
1亿 JOIN 1亿 | 58.3秒 | 10.7秒  
  
性能主要取决于参与hash JOIN的行数（通常来说一张大表，可能会有一些条件过滤掉一些数据，再参与JOIN）。   
