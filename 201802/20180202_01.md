...
10亿记录，GIN倒排索引占用空间XXGB，创建索引耗时xx分钟。
...
```substring(info,1,28)||gen_hanzi(4)||substring(info,29,28)```将得到相似度为0.75的一个新字符串。
...
```
create or replace function get_res(text, int8) returns record as $$
declare
  lim float8 := 1;
begin
  ...
  loop
    ...
    -- 查看当前阈值下，有没有相似记录
    perform select similarity(info, $1) as sml, * from tbl where info % $1 limit 1;
...
```
select * from get_res('输入搜索文本', 输入限制条数) as t(sml float8, id int, info text);
```
