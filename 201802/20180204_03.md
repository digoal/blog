...
---|---|---|---|---  
4个并行分片，每个分片返回少量数据 | 2.37 秒 | 6.99 秒 | 2.95 倍 | 几乎线性提升  
4个并行分片，每个分片返回大量数据（但是下一个NODE包括并行聚合） | 6.46 秒 | 21.7 秒 | 3.36 倍 | 几乎线性提升  
4个并行分片，每个分片返回大量数据（下一个NODE需要串行返回大量数据） | 76.5 秒 | 18.3 秒 | - 4.18 倍 | append的下一个NODE需要返回大量数据时不适合  
