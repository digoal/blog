...
假设score的取值范围是0到100，我们将score分为100段，每段1个百分点.   
...
```  
latency average = 118.613 ms  
latency stddev = 10.778 ms  
tps = 471.883670 (including connections establishing)  
tps = 471.917862 (excluding connections establishing)  
```  
  
## 小结    
1、gin_fuzzy_search_limit 起到了限流效果(具备一定随机性，并不精确)，同时性能的提升不明显(5000左右性能较佳)，并且测试中发现gin_fuzzy_search_limit设置为很小时，性能并不好。  
  
2、使用rum分段索引，可以实现高效率的过滤，按SCORE排序，得到精准的排序后的数据。如果需要更好的并发，需要更多的只读节点来支持。  
  
测试实例为6140 RMB每月的RDS PG 10最高规格。（56核, 480G内存, 2T存储.）, 当然本例还有优化空间，比如分段可以再细一些。      
  
3、本文使用的是非分区表，但是多索引按SCORE进行范围分段的方法，实现更加高效率的检索。实际使用时，对表进行分区也可以达到同样的效果。   
