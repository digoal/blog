## 阿里云 RDS PostgreSQL 高并发特性 vs PgSQL 社区版本        
...
## 背景     
PostgreSQL 是进程模型，与Oracle dedicate server一样。在非常高并发的情况下，性能会下降比较厉害，通常可以通过加连接池来解决，例如pgbouncer，但是加连接池也会带来一些问题：    
...
9、在连接数达到1.6万时，阿里云RDS PG版本，TPC-B依旧可以提供高达15万的QPS。    
    
欢迎使用阿里云RDS PG，我们会一如既往的优化，提高用户使用感受。这个特性同时支持阿里云的RDS PG以及RDS PPAS两个产品。    
