...
### 4 索引数超过4并且SIZE大于10MB的top(dml)表  
...
```  
select proname, prosrc from pg_proc where not prosecdef and proowner not in (select oid from pg_roles where rolname in ('白名单(系统用户,超级用户)'));  
```  
