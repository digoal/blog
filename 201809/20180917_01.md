...
### 步骤
1、创建slot(因为库非常大，没有SLOT的话可能会导致WAL日志被移除。如果有WAL归档的情况下，则不需要开启SLOT。)   
  
```
postgres=# select pg_create_physical_replication_slot('standby1');
 pg_create_physical_replication_slot 
-------------------------------------
 (standby1,)
(1 row)

postgres=# select pg_create_physical_replication_slot('standby2');
 pg_create_physical_replication_slot 
-------------------------------------
 (standby2,)
(1 row)
