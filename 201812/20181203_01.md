...
autovacuum_work_mem = 1G               # min 1MB, or -1 to use maintenance_work_mem  
...
# 是否启用exp = null 自动转换为 exp is null
...
### 64G内存，16核，HUGE PAGE=on，SSD机器的配置例子   
  
```  
listen_addresses = '0.0.0.0'            # what IP address(es) to listen on;  
port = 1921                             # (change requires restart)  
max_connections = 3200                  # (change requires restart)  
superuser_reserved_connections = 13      # (change requires restart)  
unix_socket_directories = '., /tmp'        # comma-separated list of directories  
unix_socket_permissions = 0700          # begin with 0 to use octal notation     
tcp_keepalives_idle = 60                # TCP_KEEPIDLE, in seconds;  
...
huge_pages = try                # on, off, or try  
max_prepared_transactions = 3200        # zero disables the feature  
...
hot_standby_feedback = off             # send info from standby to prevent  
max_logical_replication_workers = 64    # taken from max_worker_processes  
max_sync_workers_per_subscription = 12  # taken from max_logical_replication_workers  
enable_partitionwise_join = on  
enable_partitionwise_aggregate = on  
random_page_cost = 1.1                 # same scale as above  
