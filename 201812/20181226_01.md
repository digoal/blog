## PG,PPAS,oracle,mysql,ms sql,sybase 迁移到 PG, PPAS (跨版本升级)  
...
## 背景       
PG, PPAS, oracle, mysql, ms sql, sybase 迁移到 PG, PPAS (跨版本升级)，可以使用EDB的MTK工具。  
...
创建迁移源库，并生成源库128张表，每张表100万记录，一共1.28亿记录。  
