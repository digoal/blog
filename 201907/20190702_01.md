...
### 存储巨变，2.5万iops提升到100万iops，2TB提升到32TB。
...
1、选中可用区（香港），创建加密KEY。发稿截止时间，暂时只有香港REGION支持KMS加密。  
...
很多用户抱怨RDS PG的日志不友好，我们会在RDS PG上支持性能洞察，另外近期会推出log_fdw的插件，用户可以使用LOG_FDW将原始的CSV LOG取走。
