## PostgreSQL 阿里云rds pg图像识别 - pase 插件  
...
• clustering_params：对于外部聚类，该项配置为中心点文件路径。对于内部聚类，该项配置为聚类参数，格式为：clustering_sample_ratio,k，clustering_sample_ratio为库内数据用作聚类的采样比例，范围(0, 1000]内的整数，k为聚类中心数。   
...
说明：  
• base_nb_num：邻居数，第0层 ```2 * base_nb_num:```，高层为base_nb_num:。  
• ef_build：   build索引的时候的堆长度（构图时，每层使用多少个邻居点来构图），越大效果越好，build越慢。```不是存的内容，是build时构图的一个参数。```  
• ef_search：  查询的堆长度，越大效果越好，search性能越差，可在查询时指定，该处为默认值。  
