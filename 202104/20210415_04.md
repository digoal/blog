...
7、如果是分区数量导致的问题, 那么就要分析是什么分区类型, 如果是枚举或hash分区, 通常一个长连接可能每次请求的分区都是不同的分区表, 连接使用久了就会hold所有的分区元数据, 占用内存大.  如果是时间分区, 通常在日志表场景使用, 每次请求的基本上都是当前时期的分区, 所以长连接hold的分区数量不会太多, 不大会有内存消耗问题.  
    - 如果是hash或枚举分区产生的问题怎么办? 除了前面提到的6点方法, 还有2种方法, 业务层改造减少分区数量也能减少内存消耗. 或: 业务层集成pg的hash或枚举算法, 把和不同分区相关的数据操作分配到不同的线程, 从而分配到不同的数据库连接, 因此每个连接只会touch到某些特定的分区表, 不会导致每个分区都hold 所有分区表.     
