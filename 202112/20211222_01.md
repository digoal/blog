...
- 通常是末端,   
- 特殊情况: 如果当前会话已获得某个锁, 或者已经在等某个锁, 而且等待队列中有其他会话正在等待当前会话已获得的锁, 那么就插入到这个等待队列中的会话之前), 进入sleep状态.  
...
x  
  d 插入这 ...   
c (这个会话和我的会话已获得(b)或更早等待中的锁(a)冲突, 那么就把当前会话插入到它前面)    
x  
x  
```  
...
按照PG的死锁检测算法, 在等待中的会话都会每隔sleep时长(deadlock_timeout, 默认1秒)就发起死锁检测, 所以谁先检测出死锁就rollback谁(属于自杀行为).    
