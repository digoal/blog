...
未来的优化:   
- 在server中加入更多的option, 例如设置s3的参数, 连接时就默认配置好, 这样的话就可以直接查询foreign table, 不需要每次都需要通过execute接口来配置.  
  
其他测试待补充:  
- 通过duckdb_fdw写入oss
- 多个会话同时访问相同server的foreign table, 看一下是否支持. 支持的话就会方便很多, 否则要创建很多的server
