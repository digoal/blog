...
echo "#  add by digoal" >>$PG_HOME/.bashrc  
echo "alias rm='rm -i'" >>$PG_HOME/.bashrc  
echo "alias cp='cp -i'" >>$PG_HOME/.bashrc  
echo "alias ll='ls -larth'" >>$PG_HOME/.bashrc  
echo "alias mv='mv -i'" >>$PG_HOME/.bashrc  
echo "export PGHOME=/usr/lib/postgresql/14" >>$PG_HOME/.bashrc  
echo "export PATH=$PGHOME/bin:$PATH" >>$PG_HOME/.bashrc  
echo "export LD_LIBRARY_PATH=$PGHOME/lib:$LD_LIBRARY_PATH" >>$PG_HOME/.bashrc  
echo "export PGDATA=/var/lib/postgresql/14/pgdata" >>$PG_HOME/.bashrc  
echo "export PGUSER=postgres" >>$PG_HOME/.bashrc  
echo "export PGHOST=$PGDATA" >>$PG_HOME/.bashrc  
echo "export PGPORT=1921" >>$PG_HOME/.bashrc  
echo "export PGDATABASE=postgres" >>$PG_HOME/.bashrc  
echo "export LC_ALL=en_US.UTF-8" >>$PG_HOME/.bashrc  
  
chown postgres:postgres $PG_HOME/.bashrc  
```  
...
cd $TEMP_DIR  
wget https://github.com/libgd/libgd/archive/refs/tags/gd-2.3.3.tar.gz  
...
cd $TEMP_DIR  
git clone --depth 1 https://github.com/ossc-db/pg_store_plans  
cd $TEMP_DIR/pg_store_plans  
USE_PGXS=1 make  
USE_PGXS=1 make install  
  
cd $TEMP_DIR  
git clone --depth 1 https://github.com/EnterpriseDB/mongo_fdw  
...
wget https://api.pgxn.org/dist/datasketches/1.6.0/datasketches-1.6.0.zip  
...
curl -s -L http://faculty.cse.tamu.edu/davis/GraphBLAS/GraphBLAS-3.1.1.tar.gz | tar zxvf - && cd GraphBLAS-3.1.1 && make library CMAKE_OPTIONS='-DCMAKE_BUILD_TYPE=Debug' && make install  
cd $TEMP_DIR  
git clone --depth 1 --branch 22July2019 https://github.com/GraphBLAS/LAGraph.git && cd LAGraph && make -j4 library && make install  
cd $TEMP_DIR  
curl -s -L https://github.com/theory/pgtap/archive/v0.99.0.tar.gz | tar zxvf - && cd pgtap-0.99.0 && make -j4 && make install  
...
ENV DEBIAN_FRONTEND=noninteractive TEMP_DIR=/tmp ROOT_HOME=/root PG_HOME=/var/lib/postgresql PG_DATA=/var/lib/postgresql/14/pgdata  
STOPSIGNAL SIGINT  
...
# RUN CPPFLAGS="-std=c++17 -Wno-register -D_GLIBCXX_USE_CXX11_ABI=0" USE_PGXS=1 make  
# RUN CPPFLAGS="-std=c++17 -Wno-register -D_GLIBCXX_USE_CXX11_ABI=0" USE_PGXS=1 make install  
...
# docker build -t="digoal/pg14:with_exts" --no-cache .  
```  
...
- parquet_s3_fdw, 读写s3,oss对象存储和parquet文件    (未打包, 通过duckdb_fdw可以读写s3, aliyun oss, 更加快捷)  
...
- plv8, google v8存储过程和函数语言  (未打包, 未来可以通过apt安装)  
...
u8m1rp4bjqqi7h8mi0y3m8zj3  
jfkixfaojucvfqd9j7tofi69f  
qzpluuv5ihyej5r2b91q6rx39  
xcjfhzqe7oi06x9cw0lqpi0w4  
qh25unmtqql26ge8q04vo3iy2  
ovrj7q3saiwzmpvtejzsg8rl3  
onbee85ugipg5pe924a7v8r25  
6g788os41eyx4tzt6imyv66f3  
mwy0axeunpzldy4t6u44wthr3  
3wdx9bt9x4l9d3cbj1v2l3re0  
iyduajoi9a0rq0dr62833ssvd  
g4m1lcsa2orvkfkx2kj1a8n4b  
skfkwb8ayks4mjwlka8bkr4xh  
vaqfwe49q2ywrbc2od67ql99c  
js3dmaajpckxp13o44m3j9ntj  
