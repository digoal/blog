...
PostgreSQL发行版
- HaloDB, 杭州羲和数据发布的兼容PostgreSQL, MySQL, Oracle的数据库. 感谢章老哥提供二进制包和试用许可. su - halo 即可使用. 
