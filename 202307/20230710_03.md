...
这个镜像集成了177个扩展插件, 提升业务开发效率, 解决业务问题等, 涉及各个领域, 参考末尾介绍. 以及若干数据库管理工具(例如pg_rman, pgpool等).  
...
address_standardizer                            icu_ext                                         pgpool_regclass                                 postgis
"address_standardizer-3"                        imgsmlr                                         pg_prewarm                                      "postgis-3"
address_standardizer_data_us                    insert_username                                 pg_profile                                      postgis_raster
"address_standardizer_data_us-3"                intagg                                          pg_qualstats                                    "postgis_raster-3"
adminpack                                       intarray                                        pg_query_rewrite                                postgis_sfcgal
age                                             ip4r                                            pg_rational                                     "postgis_sfcgal-3"
aggs_for_vecs                                   isn                                             pg_repack                                       postgis_tiger_geocoder
amcheck                                         jsonb_plpython3u                                pgroonga                                        "postgis_tiger_geocoder-3"
anon                                            jsquery                                         pgroonga_database                               postgis_topology
autoinc                                         lo                                              pgrouting                                       "postgis_topology-3"
aws_s3                                          ltree                                           pgrowlocks                                      postgres_fdw
bloom                                           ltree_plpython3u                                pg_safer_settings                               powa
btree_gin                                       mimeo                                           pg_safer_settings_table_dependent_extension     prefix
btree_gist                                      moddatetime                                     pg_safer_settings_table_dependent_subextension  pre_prepare
citext                                          mongo_fdw                                       pg_show_plans                                   prioritize
citus                                           mysql_fdw                                       pg_similarity                                   q3c
citus_columnar                                  ogr_fdw                                         pgsodium                                        quantile
credcheck                                       old_snapshot                                    pg_sphere                                       rdkit
cube                                            oracle_fdw                                      pg_squeeze                                      refint
datasketches                                    orafce                                          pg_stat_kcache                                  roaringbitmap
dblink                                          pageinspect                                     pg_stat_monitor                                 rum
dblink_plus                                     parray_gin                                      pg_stat_statements                              seg
ddlx                                            pgagent                                         pgstattuple                                     smlar
decoderbufs                                     pgaudit                                         pg_statviz                                      sslinfo
dict_int                                        pg_bigm                                         pg_store_plans                                  tablefunc
dict_xsyn                                       pg_buffercache                                  pg_surgery                                      table_log
duckdb_fdw                                      pg_bulkload                                     pgtap                                           tcn
earthdistance                                   pg_cron                                         pg_track_settings                               tdigest
embedding                                       pgcrypto                                        pg_trgm                                         tds_fdw
extra_window_functions                          pg_curl                                         pgtt                                            temporal_tables
file_fdw                                        pg_dbms_stats                                   pg_utility_trigger_functions                    timescaledb
first_last_agg                                  pg_dirtyread                                    pg_variables                                    timescaledb_toolkit
fuzzystrmatch                                   pgfaceting                                      pg_visibility                                   toastinfo
gevel                                           pgfincore                                       pg_wait_sampling                                tsm_system_rows
h3                                              pg_freespacemap                                 pldbgapi                                        tsm_system_time
h3_postgis                                      pggraphblas                                     pljava                                          unaccent
hdfs_fdw                                        pg_hint_plan                                    pllua                                           unit
hll                                             pg_ivm                                          plluau                                          "uuid-ossp"
hnsw                                            pg_jieba                                        plpgsql_check                                   vector
hstore                                          pgjwt                                           plprofiler                                      vops
hstore_pllua                                    pgmemcache                                      plproxy                                         xml2
hstore_plluau                                   pgmp                                            plpython3u                                      zhparser
hstore_plpython3u                               pg_net                                          plr                                             
http                                            pgpool_adm                                      pointcloud                                      
hypopg                                          pgpool_recovery                                 pointcloud_postgis 
```  
...
curl -Z --connect-timeout 120 -m 36000 --retry 12000 --retry-delay 5 --retry-max-time 1200 -L https://github.com/duckdb/duckdb/releases/download/v0.8.1/libduckdb-linux-amd64.zip -o libduckdb-linux-amd64.zip  
  
curl -Z --connect-timeout 120 -m 36000 --retry 12000 --retry-delay 5 --retry-max-time 1200 -L https://github.com/duckdb/duckdb/releases/download/v0.8.1/duckdb_cli-linux-amd64.zip -o duckdb_cli-linux-amd64.zip  
...
# wget -T 36000 -t 0 --waitretry=5 https://github.com/duckdb/duckdb/releases/download/v0.8.1/libduckdb-linux-amd64.zip  
# curl -Z --connect-timeout 120 -m 36000 --retry 12000 --retry-delay 5 --retry-max-time 1200 -L https://github.com/duckdb/duckdb/releases/download/v0.8.1/libduckdb-linux-amd64.zip -o libduckdb-linux-amd64.zip  
...
COPY GraphBLAS-3.1.1.tar.gz libduckdb-linux-aarch64.zip duckdb_cli-linux-aarch64.zip cmake-3.27.4.tar.gz mongo-c-driver-1.17.3.tar.gz json-c-0.15-20200726.tar.gz $TEMP_DIR/
...
# RUN curl -Z --connect-timeout 120 -m 36000 --retry 12000 --retry-delay 5 --retry-max-time 1200 -L https://github.com/duckdb/duckdb/releases/download/v0.8.1/duckdb_cli-linux-amd64.zip -o duckdb_cli-linux-amd64.zip  
...
docker build --platform=linux/amd64 -t="digoal/pg14:with_exts" . 2>&1 | tee ./logs/build.log  
  
# docker build --platform=linux/amd64 -t="digoal/pg14:with_exts" --no-cache . 2>&1 | tee ./logs/build.log  
```  
...
```  
docker run --platform=linux/amd64 -d -it -P --cap-add=SYS_PTRACE --cap-add SYS_ADMIN --privileged=true --name pg --shm-size=1g digoal/pg14:with_exts  
...
- pg_plan_inspector, 复杂SQL执行计划优化修正插. 使用机器学习的方法对收集到的SQL和执行计划等信息进行分析, 从而提升性能.  
