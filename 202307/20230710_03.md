...
这个镜像集成了190个扩展插件, 提升业务开发效率, 解决业务问题等, 涉及各个领域, 参考末尾介绍. 以及若干数据库管理工具(例如pg_rman, pgpool等).  
...
address_standardizer                            insert_username                                 pgpool_regclass                                 pointcloud
"address_standardizer-3"                        intagg                                          pg_prewarm                                      pointcloud_postgis
address_standardizer_data_us                    intarray                                        pg_profile                                      postgis
"address_standardizer_data_us-3"                ip4r                                            pg_qualstats                                    "postgis-3"
adminpack                                       isn                                             pg_query_rewrite                                postgis_raster
age                                             jsonb_plpython3u                                pg_rational                                     "postgis_raster-3"
aggs_for_vecs                                   jsquery                                         pg_repack                                       postgis_sfcgal
amcheck                                         lantern                                         pgroonga                                        "postgis_sfcgal-3"
anon                                            lantern_extras                                  pgroonga_database                               postgis_tiger_geocoder
autoinc                                         lo                                              pgrouting                                       "postgis_tiger_geocoder-3"
aws_s3                                          ltree                                           pgrowlocks                                      postgis_topology
bloom                                           ltree_plpython3u                                pg_safer_settings                               "postgis_topology-3"
btree_gin                                       mimeo                                           pg_safer_settings_table_dependent_extension     postgres_fdw
btree_gist                                      moddatetime                                     pg_safer_settings_table_dependent_subextension  powa
citext                                          mongo_fdw                                       pg_search                                       prefix
citus                                           mysql_fdw                                       pgsentinel                                      pre_prepare
citus_columnar                                  ogr_fdw                                         pg_show_plans                                   prioritize
columnar                                        old_snapshot                                    pg_similarity                                   q3c
credcheck                                       oracle_fdw                                      pgsodium                                        quantile
cube                                            orafce                                          pg_sphere                                       rdkit
datasketches                                    pageinspect                                     pg_squeeze                                      refint
dblink                                          parquet_fdw                                     pg_stat_kcache                                  roaringbitmap
dblink_plus                                     parray_gin                                      pg_stat_monitor                                 rum
ddlx                                            pgagent                                         pg_stat_statements                              seg
decoderbufs                                     pgaudit                                         pgstattuple                                     smlar
dict_int                                        pg_bigm                                         pg_statviz                                      sqlite_fdw
dict_xsyn                                       pg_bm25                                         pg_store_plans                                  sslinfo
duckdb_fdw                                      pg_buffercache                                  pg_surgery                                      tablefunc
earthdistance                                   pg_bulkload                                     pgtap                                           table_log
embedding                                       pg_cron                                         pg_task                                         tcn
extra_window_functions                          pgcrypto                                        pg_tiktoken                                     tdigest
file_fdw                                        pg_curl                                         pg_track_settings                               tds_fdw
first_last_agg                                  pg_dbms_stats                                   pg_trgm                                         temporal_tables
fuzzystrmatch                                   pg_dirtyread                                    pgtt                                            timescaledb
gevel                                           pgfaceting                                      pg_utility_trigger_functions                    timescaledb_toolkit
h3                                              pgfincore                                       pg_uuidv7                                       toastinfo
h3_postgis                                      pg_freespacemap                                 pg_variables                                    tsm_system_rows
hdfs_fdw                                        pggraphblas                                     pg_visibility                                   tsm_system_time
hll                                             pg_hint_plan                                    pg_wait_sampling                                unaccent
hnsw                                            pg_ivm                                          pldbgapi                                        unit
hstore                                          pg_jieba                                        pljava                                          "uuid-ossp"
hstore_pllua                                    pgjwt                                           pllua                                           vector
hstore_plluau                                   pgmemcache                                      plluau                                          vops
hstore_plpython3u                               pgmp                                            plpgsql_check                                   xml2
http                                            pg_net                                          plprofiler                                      zhparser
hypopg                                          pg_onnx                                         plproxy                                         zson
icu_ext                                         pgpool_adm                                      plpython3u                                      
imgsmlr                                         pgpool_recovery                                 plr                                             
```  
...
curl -Z --connect-timeout 120 -m 36000 --retry 12000 --retry-delay 5 --retry-max-time 1200 -L https://github.com/duckdb/duckdb/releases/download/v0.9.1/duckdb_cli-linux-amd64.zip -o duckdb_cli-linux-amd64.zip  
...
apt-get install -y curl libicu-dev icu-devtools libbison-dev libfl-dev git libreadline-dev libedit-dev g++ make cmake man-db dnsutils clang libssl-dev default-jdk strace gdb libsqlite3-dev  
...
WORKDIR $PG_HOME  
# RUN wget -T 36000 -t 0 --waitretry=5 https://github.com/duckdb/duckdb/releases/download/v0.9.1/duckdb_cli-linux-amd64.zip  
# RUN curl -Z --connect-timeout 120 -m 36000 --retry 12000 --retry-delay 5 --retry-max-time 1200 -L https://github.com/duckdb/duckdb/releases/download/v0.9.1/duckdb_cli-linux-amd64.zip -o duckdb_cli-linux-amd64.zip  
