...
### 标签                      
PostgreSQL , PolarDB , 全文检索 , gin , bm25 , elastic search , pg_sparse , ann , sparse vectors           
...
pg_bm25, pg_search, pg_sparse 将被集成到这两个学习镜像中:  
...
# pgrx版本请参考不同版本的pg_bm23 cargo.toml文件: https://github.com/paradedb/paradedb/blob/dev/pg_bm25/Cargo.toml  
cargo install --locked --version 0.11.1 cargo-pgrx     
cargo pgrx init --pg14=`which pg_config`     
      
# build and install pg_bm25, pg_search, pg_sparse    
su - root
cargo install --locked --version 0.11.1 cargo-pgrx
...
```  
     ...
     Writing SQL entities to /usr/share/postgresql/14/extension/pg_bm25--0.4.3.sql
...
#     Copying control file to /usr/share/postgresql/14/extension/pg_sparse.control
#     Copying shared library to /usr/lib/postgresql/14/lib/pg_sparse.so
#     Writing SQL entities to /usr/share/postgresql/14/extension/pg_sparse--0.0.0.sql
```  
