...
# 或socks5h 包括dns也走socks解析
# echo "export all_proxy=socks5h://127.0.0.1:$locallistenport"
...
if pgrep -x "ssh" >/dev/null && [ $(ps -efw|grep -v grep|grep ssh|grep CqtfnN|grep -v CqtgfnN|grep -c $locallistenport) -eq 1 ]
then
  echo "ssh 进程正在运行" 
else
  ssh_func
...
和这篇的dockerproxy.sh配合使用. [《通过海外服务器配置squid http/https proxy代理, 本地ssh建隧道 端口映射, 使用http/https proxy代理配置拉取docker hub镜像, 手机youtube app登陆等》](../202407/20240704_01.md)  
```
vi a.sh
#!/bin/bash
...
kill $(ps aux|grep bash|grep proxy.sh|grep -v grep|awk '{print $2}')
kill $(ps aux|grep ssh|grep CqtfnN|grep -v CqtgfnN|grep 1111|grep -v grep|awk '{print $2}')
kill $(ps aux|grep ssh|grep CqtgfnN|grep 22222|grep -v grep|awk '{print $2}')
