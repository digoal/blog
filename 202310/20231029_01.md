...
1、macOS  
2、chrome 浏览器  
3、在aliyun选购1台新加坡(或其他oversea region)的ecs(1c, 0.5G 的突发型即可), 选择使用debian 11.7系统, 开启公网(选择固定带宽模式, 1Mib足够使用), 20GiB云盘, 关闭其他任何可能产生费用的功能. 最终价格约1700元(5年).      
- 记下公网IP, root密码.     
...
/usr/bin/expect <<-EOF  
spawn ssh -CqtfnN -c aes128-ctr -o "MACs umac-64@openssh.com" -D 127.0.0.1:1111 $user@$host -p $port  
expect {  
...
spawn ssh -C -o TCPKeepAlive=yes -o ServerAliveInterval=15 -o ServerAliveCountMax=300000 $user@$host -p $port  
expect {  
"yes/no" { send "yes\r"; exp_continue }  
"password:" { send "$pwd\r" }  
}  
...
macOS, 开启cli代理的情况下, 进行速度测试:  我的Ecs带宽是1Mb, 这里就限制1Mb/8=256kBytes/s.     
```  
export all_proxy=socks5://127.0.0.1:1111    
  
cd ~/Downloads  
  
curl --limit-rate 256k -Z --connect-timeout 120 -m 36000 --retry 12000 --retry-delay 5 --retry-max-time 1200 -O -L https://github.com/duckdb/duckdb/releases/download/v0.9.1/duckdb_cli-osx-universal.zip   
...
speed没有达到`256kB/s`, 暂时还不知道为什么跑不满带宽.    
...
用safari 浏览器下载 SwitchyOmega_Chromium.crx 插件:    
...
2、macOS git-push , git-pull 使用ssh隧道(未测试):    
```  
vi ~/.ssh/config  
...
Host GitHub.com  
        ...
        Port    22  
        # Linux/Mac:  
        Proxycommand    /usr/bin/ncat --proxy 127.0.0.1:1111 --proxy-type socks5 %h %p  
  
        # Windows:  
        # Proxycommand   connect -S 127.0.0.1:1111 %h %p  
```  
