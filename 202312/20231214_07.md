...
- vid 已读列表 (vid `1 ~ 300万` 取值空间, 对应前面写入的300万个视频).   
    - 重度用户, 每个用户36.5万个已观看视频. userid号段: 1 ~ 10w 
    - 中度用户, 每个用户5万个已观看视频. userid号段: 10w ~ 500w 
    - 一般用户, 每个用户5千个已观看视频. userid号段: 500w ~ 1000w 
...
-- 重度用户, userid号段: 1 ~ 10w 
-- 每个用户的已观看视频都是36.5万个. 
select generate_series(1,100000), ceil(random()*360),      
...
insert into users    
-- 中度用户, userid号段: 10w ~ 500w  
-- 每个用户的已观看视频都是5万个. 
select generate_series(100001, 5000000), ceil(random()*360),      
-- 每个用户20个标签 (标签取值空间1 ~ 100), limit大于0的标签5个(和为100, 也就是说每次请求推荐100条未读视频).      
array(      
  select row(ceil(random()*100), random()*10, 40)::tag_score       
  union all      
...
-- 中度用户
-- 每个用户的已观看视频都是5万个. (vid 取值空间 1 ~ 300万, 对应前面写入的300万个视频).     
-- 聚合为jsonb数组, 每个vids列表代表 mod(x,64)=i 的vids.  对应partial index, 用于减少已读的cpu过滤, 提升搜索性能.      
(    
select jsonb_agg(x) as readlist from    
  (      
...
insert into users    
-- 一般用户, userid号段: 500w ~ 1000w 
-- 每个用户的已观看视频都是5000个. 
select generate_series(5000001, 10000000), ceil(random()*360),      
-- 每个用户20个标签 (标签取值空间1 ~ 100), limit大于0的标签5个(和为100, 也就是说每次请求推荐100条未读视频).      
array(      
  select row(ceil(random()*100), random()*10, 40)::tag_score       
  union all      
...
-- 一般用户
-- 每个用户的已观看视频都是5000个. (vid 取值空间 1 ~ 300万, 对应前面写入的300万个视频).     
-- 聚合为jsonb数组, 每个vids列表代表 mod(x,64)=i 的vids.  对应partial index, 用于减少已读的cpu过滤, 提升搜索性能.      
(    
select jsonb_agg(x) as readlist from    
  (      
    select array (select x from     
                     (select ceil(random()*3000000)::int8 x from generate_series(1,5000)) t     
                   where abs(mod(hashint8(x),64)) = i     
                 ) x     
    from generate_series(0,63) i     
  ) t    
...
以上导入的数据, 相当于平台已经拥有 3065 亿次有效视频观看, 仅流量广告费就 3065 亿:    
- 3065 = `36.5*10 + 5*490 + 0.5*500` 
