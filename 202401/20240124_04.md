...
- 需要在ECS系统中安装docker server/desktop, 以便运行打包好的 pfs/PolarDB 容器. <b> 注意这不是必须的, 只是为了学习方便. 如果你需要在物理机或ECS中直接部署PolarDB, 可以参考 [《PolarDB 100 问 | 如何构建PolarDB Docker镜像 OR 本地编译PolarDB?》](../202412/20241205_02.md) 对应操作系统Dockerfile的内容, 把依赖包装好就可以编译PolarDB源码. </b>   
