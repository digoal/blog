...
ALTER TABLE nation SET (px_workers = 10);    
ALTER TABLE region SET (px_workers = 10);    
ALTER TABLE supplier SET (px_workers = 10);    
ALTER TABLE part SET (px_workers = 10);    
ALTER TABLE partsupp SET (px_workers = 10);    
ALTER TABLE customer SET (px_workers = 10);    
ALTER TABLE orders SET (px_workers = 10);    
ALTER TABLE lineitem SET (px_workers = 10);    
\q  
```  
...
```  
cd ~/tpch-dbgen    
./build.sh --user=postgres --db=postgres --host=/tmp --port=5432 --run=17 --option="set polar_enable_px = off; set work_mem = '16MB';"     
```  
  
关闭ePQ测试耗时超过40分钟都没有执行完成, 果断放弃.    
