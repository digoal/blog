...
- 开启PolarDB预分配功能(使用共享存储时效果比较明显, 减少IO次数, 降低云盘IO延迟带来的性能损耗)  
  
查询加速:     
- 使用单机并行, 通过修改表的配置和参数可启用强制并行度. 参考: [《PostgreSQL 11 并行计算算法，参数，强制并行度设置》](../201812/20181218_01.md)     
- 使用多机并行, PolarDB的配置. 参考: [《开源PolarDB|PostgreSQL 应用开发者&DBA 公开课 - 5.5 PolarDB开源版本必学特性 - PolarDB 特性解读与体验》](../202401/20240125_07.md)    
- 开启PolarDB预读功能(使用共享存储时效果比较明显, 减少IO次数, 降低云盘IO延迟带来的性能损耗)  
