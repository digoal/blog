...
以macOS宿主机(macBook pro M2 16G 512G, docker资源限制4Ccpu 8Gmem 0Gswap)为例, 测试一下tpch 20G 导入性能, 挂载宿主机目录`~/pb2024`到容器中使用, 在生成tpch原始数据时可能性能会更好一点, 同时将PolarDB集群也初始化到宿主机中, 还可以防止测试过程中容器文件过大.    
