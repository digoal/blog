...
#### 常见错误排查
如果在编译并启动PolarDB集群时报内存不足的错误, 可能是docker desktop的内存资源限制太少了, 可以修改一下(修改配置后需要重启docker daemon. 例如Linux: `systemctl restart docker`). 例如 16G内存的Mac我配置了limit 8G内存和0G swap. 配置请参考docker手册:   
