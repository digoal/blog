## 通过http / https proxy代理配置拉取docker hub镜像    
...
bypass proxy settings for these hosts/domains: 不使用代理的registry  :   registry.cn-hangzhou.aliyuncs.com
```
...
```  
ssh -C -f -N -g -L 22222:127.0.0.1:3128  user@xxx.xxx.xxx.xxx    
```  
...
```  
ssh -CqtgfnN -c chacha20-poly1305@openssh.com -o "MACs umac-64@openssh.com" -o TCPKeepAlive=yes -o ServerAliveInterval=10 -o ServerAliveCountMax=300000 -L 22222:127.0.0.1:3128 $user@$host -p $port    
```
...
/usr/bin/expect <<-EOF
spawn ssh -CqtgfnN -c aes128-ctr -o "MACs umac-64@openssh.com" -o TCPKeepAlive=yes -o ServerAliveInterval=10 -o ServerAliveCountMax=300000 -L $localmapport:$remoteproxyip:$remoteproxyport $user@$host -p $port 
expect {
