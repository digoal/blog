## AI大模型+全文检索+tf-idf+向量数据库+我的文章 系列之6 - 科普 : 大模型到底能干什么? 模型已经出现等级划分了? 专业术语? 资源消耗?    
...
需求和模型选择:  
- 1、支持的上下文大小 32k, 128k, 256k, ... 通常越大的需要更多的算力, 推理能力越强, 代码生成的能力也越强.  
- 2、Simple tasks that one can do in bulk (Classification, Customer Support, or Text Generation) are powered by Mistral Small.
- 3、Intermediate tasks that require moderate reasoning (Data extraction, Summarizing a Document, Writing emails, Writing a Job Description, or Writing Product Descriptions) are powered by Mistral 8x22B.
- 4、Complex tasks that require large reasoning capabilities or are highly specialized (Synthetic Text Generation, Code Generation, RAG, or Agents) are powered by Mistral Large.
- 5、专业模型, coding. 
- 6、专业模型, embedding. 
