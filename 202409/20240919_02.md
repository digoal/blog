...
## 简介  
PostgreSQL heap表的显著特点是以无序、行的形式存储数据. 一条记录为一个整体插入到某个block的空闲位置(特殊情况: 当一条记录的变长类型字段内容超过toast_tuple_target尺寸时, 这些字段的内容将会被存储到toast空间, 这个以后再细说.), 如果只有insert操作, 看起来就像在数据文件末尾以append only的形式追加数据.    
