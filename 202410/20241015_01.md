...
例子2可以skip 的 data page:   
- 1、通过 column index 获取到满足范围条件的page, 例如: name_min = 'abc', name_max = 'hello'.   
- 2、还能缩小范围吗? 可以: 获取dictionary page内的值, 如果无digoal.    
- 3、data page 内name的值只有dictionary内的value. (没有未字典化的值) ; -- 假设在column index里用1个bit存储了这个信息(该page/pages是否只包含dictionary page内的值).   
...
如果N个data page对应一个SBBF, 查找col=?时, 只要在block_check返回false就可以跳过这些data page.    
