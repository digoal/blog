...
- 根据SQL的执行计划来进行分析, 看SQL的耗时卡点是什么, 针对性的进行优化. 例如查看`sql/1.sql`的执行计划和各个耗时点位:    
    ...
    cd /tmp/PolarDB-for-PostgreSQL/
    > ./explain_analyze.log
    for ((i=1;i<=22;i++)) do echo "Query: $i" >> ./explain_analyze.log ; echo "`sed 's@^select@explain (analyze,verbose,timing,costs,buffers) select@' sql/1.sql`" | psql -f - >> ./explain_analyze.log ; done 
    ```  
