...
create index on lineitem (l_returnflag,l_linestatus) include (l_quantity,l_extendedprice,l_discount,l_tax) where l_shipdate <= date '1998-12-01' - interval '118' day;
set enable_seqscan to default;

-- 3 
create index on customer (c_custkey) where c_mktsegment = 'BUILDING';
create index on orders (o_custkey) include (o_orderdate,o_shippriority) where o_orderdate < date '1995-03-07';
create index on lineitem (l_orderkey) include (l_extendedprice,l_discount) where l_shipdate > date '1995-03-07';

-- 4 
create index on lineitem (l_orderkey) where l_commitdate < l_receiptdate;
create index on orders (o_orderpriority) where o_orderdate >= date '1994-02-01' and o_orderdate < date '1994-02-01' + interval '3' month;

-- 5  
set enable_nestloop = off;
set enable_bitmapscan = off;
create index on orders (o_custkey) include (o_orderkey) where o_orderdate >= date '1993-01-01' and o_orderdate < date '1993-01-01' + interval '1' year;  
set enable_nestloop to default;
set enable_bitmapscan to default;

-- 8  
set enable_nestloop = off;
create index on orders (o_orderkey) where o_orderdate between date '1995-01-01' and date '1996-12-31';
create index on part (p_partkey) where p_type = 'ECONOMY BRUSHED BRASS';
set enable_nestloop to default;

-- 9
set enable_nestloop = off;
create index on part (p_partkey) where p_name like '%sandy%';
set enable_nestloop to default;

-- 10 
create index on lineitem (l_orderkey) include (l_extendedprice,l_discount) where l_returnflag = 'R';
create index on orders (o_orderkey) include (o_custkey) where o_orderdate >= date '1993-12-01' and o_orderdate < date '1993-12-01' + interval '3' month;

-- 15
create index on lineitem (l_suppkey) include (l_extendedprice,l_discount) where l_shipdate >= date '1996-09-01' and l_shipdate < date '1996-09-01' + interval '3' month;

-- 17
create index on part (p_partkey) where p_brand = 'Brand#35' and p_container = 'JUMBO PKG';
create index on lineitem (l_partkey) include (l_quantity);

-- 18
create index on lineitem (l_orderkey) include (l_quantity);

-- 20
create index on lineitem (l_partkey,l_suppkey) where l_shipdate >= date '1997-01-01' and l_shipdate < date '1997-01-01' + interval '1' year;
create index on part (p_partkey) where p_name like 'wheat%';

-- 21
create index on lineitem (l_orderkey) include (l_suppkey) where l_receiptdate <> l_commitdate;
create index on lineitem (l_orderkey) include (l_suppkey);
create index on orders (o_orderkey) where o_orderstatus = 'F';
```
