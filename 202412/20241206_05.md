...
- 根据SQL的执行计划来进行分析, 看SQL的耗时卡点是什么, 针对性的进行优化. 例如查看`sql/1.sql`的执行计划和各个耗时点位:    
    ...
    或者通过下列方式把所有query都跑一遍:
    cd /tmp/PolarDB-for-PostgreSQL/
    > ./explain_analyze.log
    for ((i=1;i<=22;i++)) do echo "$i" >> ./explain_analyze.log ; echo "explain (analyze,verbose,timing,costs,buffers) `cat sql/$i.sql`" | psql -f - >>./explain_analyze.log ; done
    ```  
