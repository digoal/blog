...
微调不同的模型, 需要不同的jsonl内容格式, 请参考模型说明(通常`ollama` 对应模型 `models`页面中有相关的介绍), 或者问chatgpt.      
...
```
千万不要下载 consolidated.safetensors , 微调时会导致报错:
...
执行微调, 参数设置建议可参考 https://github.com/ml-explore/mlx-examples/tree/main/lora#memory-issues     
```  
cd ~/mlx-examples/lora  
