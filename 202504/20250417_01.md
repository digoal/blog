...
```
update wiki set vec = openai.vector(content, 'mxbai-embed-large:latest');
update wiki_cks set vec = openai.vector(content, 'mxbai-embed-large:latest');
update wiki_cks_qa set vec = openai.vector(qa, 'mxbai-embed-large:latest');
```
...
创建向量字段索引, 注意, 召回率不仅和创建时的参数有关, 还和查询时的参数有关. 参考如下文章  
- [《头大! 索引扫描和全表扫描结果不一样, 这向量数据库还能用? 教你一招大幅提升召回率(recall)》](../202404/20240417_01.md)    
```  
set max_parallel_maintenance_workers =4;    
set maintenance_work_mem ='1GB';    
ALTER TABLE wiki SET (parallel_workers = 4);    
ALTER TABLE wiki_cks SET (parallel_workers = 4);    
ALTER TABLE wiki_cks_qa SET (parallel_workers = 4);    
