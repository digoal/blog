...
- 采用RaBitQ量化, 减少计算量, 32bit -> 1bit . 参考论文: [《AI论文解读 | 向量ANNs方法之 RaBitQ: Quantizing High-Dimensional Vectors with a Theoretical Error Bound for Approximate Nearest Neighbor Search》](../202504/20250427_01.md)   
- ranker阶段时按32bit精度计算  
- ivf 分区  
- 支持GPU, 支持外部build索引, 兼容PostgreSQL数据文件, 可直接导入.   
- 建议先生成ivf的中心点集群(如使用kmeans生成), 例如有几十万条数据之后再build index 效果更佳.  
