...
12、在reorder之前, 有个参数控制候选向量的限制条数. 限制太小可能导致召回精度大幅降低.  还有一个方法可以在有限reorder条数的情况下, 保证召回精度, 例如增加一个限制score阈值, 相似度在xx范围内的向量才会成为reorder的候选向量, 这样既能保证性能还可以保证召回精度.  这个只是个人想法, 可能需要代码实现.  
