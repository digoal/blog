...
## 背景      
说到湖仓大家第一时间会想到什么? 存算分离? OLAP数据库(在线热存)+对象存储(离线冷存)架构? 曾经确实很棒很不错(有运算效率、也有大肚量).   
  
但是, 也许是时候说再见了, 因为这种架构对于大多数企业来说太贵、太笨拙.   
  
DuckDB凭借优异的性能跻身数据计算能力第一梯队, DeepSeek都对之垂涎(在其基础之上发布smallpod用于模型训练), 关键是DuckDB这么牛也就算了, 它还小巧灵活, 2012年的mac book居然可以跑SF=1000的tpc-H, 还能全跑通, 打得大数据满地找牙.  
