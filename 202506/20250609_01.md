...
## 测量排序性  
选择排序顺序最有效的方法是模拟生产工作负载，就像上面的实验一样。然而，这并不总是可行或容易做到的。相反，我们可以测量数据集在目标列上的排序效果。我们将使用的指标是“每个值的行组数量” riw group 。解释一下，为了使选择性查询有效，每个被过滤的值应该只出现在少数行组中。行组数量越少越好！然而，当这个指标低于 DuckDB 使用的线程数时，收益可能会递减。  
