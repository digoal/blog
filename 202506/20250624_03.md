...
左边为HNSW索引, 右图为IVF索引结构.   
  
假设星号为where条件输入的点(目标是要找其附近的K个近邻点), 顶层的另一个点是搜索时选择的入口点(通常只有一个点).     
  
下面以PostgreSQL pgvector插件的hnsw索引为例进行分析.  
...
- **底层（第0层）**：包含所有向量节点，连接数最多（2\*m个连接）  
- **上层（第1层及以上）**：包含部分节点，连接数较少（m个连接）  
...
SET hnsw.scan_mem_multiplier = 2;     -- 增加扫描使用的内存倍数  
```  
