...
论文中的 **Table 1**  详细列举了不同谓词的选择因子估算方法，这里举几个例子： ![pic](20250930_01_pic_001.jpg)  ![pic](20250930_01_pic_002.jpg)  
  ...
  * `column = value`:  $F \approx 1 / \text{ICARD}(\text{columnIndex})$ 。 假设“城市”列有100个不同的城市，那么查询 `城市='北京'` 的选择因子就是1/100。这基于一个均匀分布的假设 。
  * `column1 = column2` (用于Join): $F \approx 1 / \max(\text{ICARD}(\text{col1Index}), \text{ICARD}(\text{col2Index}))$ 。
