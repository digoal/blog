...
课程约32学时. 
- 第一部分, 注重数据库基本操作以及AI应用的架构与实操, 学习Docker容器的使用, 学习数据库的基操和压力测试场景的设计和实践. 通过ollama和Dify体验和理解大模型以及AI应用的设计、数据库在AI应用中的作用. 使用AI Agent(qwen3 cli)开发数据库插件, 使用数据库开发工具Navicat/pgAdmin ;
- 第二部分, 在PostgreSQL和DuckDB数据库上进行实操, 聚焦数据库在AI 搜索中最核心的几个方面: **文本切分、向量化、语义搜索、关键词检索、标量检索、混合搜索、召回率、相关性、重排、图式搜索** ;
- 第三部分, 通过一场竞赛, 回顾课程核心内容 ;  
  
掌握这些内容就掌握了常用AI 应用的使用、AI 应用的核心原理和背后的关键数据库技术, 不管未来AI 应用形态如何变化, 都可以快速上手.  
  
**课程内容会不断进化: 将来会补充多模态( https://github.com/vllm-project/vllm-omni )、Skills( https://agentskills.io/home )、国产开源数据库、生态工具使用、AI4DB、DB4AI等.**  
    
[**欢迎高校联系合作**](https://mp.weixin.qq.com/s/_9m23GccQVFvZ2h5TKEwdQ)   
...
- docker, 管理容器的工具. 
- docker image(镜像), 将应用和依赖环境整体集成在一起的最小包裹. 
- docker container(容器), 使用docker 镜像运行的应用. 
- PowerShell, windows 的命令行, 比cmd更复杂也更高级.
- cmd, windows 的命令行. 较为建议好入手. 使用时注意区分在哪个环境下, 如果执行命令报错, 先分析是不是环境不对.
- shell, 命令行界面统称, 例如Linux bash,sh,zsh用来执行操作系统命令. PostgreSQL psql用来执行PG SQL, DuckDB CLI用来执行DuckDB SQL.
- 数据库 shell, 数据库命令行界面, 例如执行 psql/duckdb 可以进入到 PostgreSQL/DuckDB 的数据库命令行. 用来执行SQL.
- 大模型, 可运行推理、embedding(文本嵌入)、ranking 排序等任务
- huggingface, 开源大模型集市
- ollama, 一种管理大模型的工具
- dify, 一种可自定义 AI 工作流和智能体的工具
- qwen3 cli, 一种AI coding工具
- Navicat, 一种支持多种数据库的GUI开发工具
- pgAdmin, 一种PG数据库GUI开发工具
- embedding, 将本文/图像等内容转换成大模型可以理解的向量
- rerank, 对结果和查询语句的向量进行相关性打分, 重新排序, 获得更好的相关性结果顺序
- 向量, 多个浮点数组成的高维数组
- 稠密向量, 多个浮点数组成的高维数组, 每个维度都有值
- 稀疏向量, 多个整型值组成的超高维数组, 不是每个维度都有值. 通常用来表示有限值中的若干个值, 例如用稀疏向量来表示字典中的若干个字.  
- 量化, 降低向量精度的技术, 例如将float32降精度到float16. 牺牲精度换取更高的性能, 更少的内存占用和更小的计算量.
- 二值量化, 将浮点数量化为二进制表示. 大于0用1表示否则用0表示. 
- SBQ 量化技术, 将浮点数量化为任意bits的技术, 需先对每个维度的采样数据进行聚集运算, 找出若干个聚集点, 每个聚集点用一个量化后的值表示. 将所有浮点数都映射到最近的聚集点(量化后的值)来进行量化.  
...
由于使用了宿主机目录作为外部卷, 要完全删除容器, 需要在`rm`容器后, 同时删除容器在宿主机上对应的卷目录. 否则再次用同样的命令创建PG容器时, 你进入容器后会发现之前操作的数据还在库里.   
...
[《大学生数据库实践课: 14.3 如何计算 bm25 得分》](../202601/20260104_01.md)
...
- [《AI论文解读 | When to use Graphs in RAG: A Comprehensive Analysis for Graph Retrieval-Augmented Generation》](../202509/20250904_01.md)  
...
## 课后大作业  
[《大学生数据库实践课 : 课后大作业 & 竞赛》](../202512/20251217_02.md)  
