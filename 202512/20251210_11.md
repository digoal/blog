...
这张图展示了使用内置 TPC-B 测试场景的三个核心步骤。

```mermaid
graph LR
    A[1\. 初始化 -i] -->|创建标准表| B(pgbench_accounts/branches/...)
    B --> C[2\. 运行测试]
    C -->|参数配置| D{并发与时间}
    D -->| -c| D1[并发客户端数]
    D -->| -t / -T| D2[事务数或持续时间]
...
根据文章第二部分整理的从“设计”到“执行”的详细步骤，适合学生复习实验流程。

```mermaid
sequenceDiagram
    participant U as 开发者/学生
    participant DB as PostgreSQL 数据库
    participant PB as pgbench 工具

    Note over U, DB: 第一步：环境准备
    U->>DB: 创建业务表 (CREATE TABLE)
    U->>DB: 创建索引 (CREATE INDEX)
    U->>DB: 插入测试数据 (generate_series)
    U->>DB: 更新统计信息 (ANALYZE)

    Note over U, PB: 第二步：脚本编写
    U->>U: 编写 .sql 脚本 (变量设置 \set)
    
    Note over PB, DB: 第三步：执行压测
    U->>PB: 执行 pgbench -f script.sql
    loop 压力测试中
        PB->>DB: 开启事务 (BEGIN)
        PB->>DB: 执行业务 SQL (SELECT/INSERT/UPDATE)
        PB->>DB: 提交事务 (COMMIT)
    end
    PB-->>U: 返回测试报告
```
...
### 4. 压测脚本 (ecommerce_test.sql) 逻辑流

这张图展示了文章中“电商订单场景”自定义脚本的内部执行逻辑。

```mermaid
flowchart TD
    Start([开始事务 BEGIN]) --> Var[设置随机变量: pid, qty, aid]
    Var --> Q1[1\. 查询商品信息 SELECT]
    Q1 --> Q2[2\. 检查库存并创建订单 INSERT]
    Q2 --> Q3[3\. 更新商品库存 UPDATE]
    Q3 --> Q4[4\. 查询最新订单状态 SELECT]
    Q4 --> End([提交事务 COMMIT])
