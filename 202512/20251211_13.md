...
## 背景  
参考此文, 构建实验数据, 调整索引参数观察性能效果. 
- [《向量教学例子》](../202512/20251217_03.md)  
...
这个流程图帮助学生建立全局观，明确从拉取数据到最终获取 QPS/召回率报告的每一个步骤。

```mermaid
graph TD
    A[1\. 准备阶段] -->|wget| A1(下载 HDF5 数据集)
    A1 --> A2(配置 Python 虚拟环境)
    
    A --> B[2\. 数据库初始化]
    B --> B1(创建用户与 DBNAME)
    B1 --> B2(CREATE EXTENSION vector)
    B2 --> B3(创建 vector_table)
    
    B3 --> C[3\. 数据导入]
    C --> C1(运行 load.py 批量插入)
    
    C1 --> D[4\. 索引构建]
    D --> D1{选择索引类型}
    D1 -->|快速构建| D2(IVFFlat: 设置 lists)
    D1 -->|高质量查询| D3(HNSW: 设置 m, ef_c)
...
实验中涉及到了特殊的存储设置（STORAGE PLAIN），这张图解释了初始化时的核心 SQL 命令关系。

```mermaid
flowchart LR
    subgraph DB_Level [数据库层]
        DB[TestDB] --- User([TestUser: Superuser])
    end

    subgraph Table_Level [表结构层]
        T[vector_table] --> ID[id: bigserial]
        T --> EMB[embedding: vector]
        
        EMB --> S[STORAGE PLAIN]
        note["注意: SET STORAGE PLAIN<br/>可以优化向量字段的访问性能"]
    end

    subgraph Ext_Level [扩展插件]
        V[pgvector 扩展]
    end
...
### 3. 索引构建与查询参数调优（实验核心对比）

这是学生在实验中需要反复“调参”的部分，图表对比了 HNSW 和 IVFFlat 的关键参数。

```mermaid
graph LR
    subgraph HNSW_Tuning [HNSW 调优]
        direction TB
        H1["构建参数: m, ef_construction"]
        H2["查询参数: hnsw.ef_search"]
        H1 -->|影响| H3[构建时间与精度上限]
        H2 -->|影响| H4[查询速度与实时召回率]
    end

    subgraph IVFFlat_Tuning [IVFFlat 调优]
        direction TB
        I1["构建参数: lists (聚类数)"]
        I2["查询参数: ivfflat.probes"]
        I1 -->|影响| I3[聚类粒度]
        I2 -->|影响| I4[探测范围]
    end
...
1. **距离操作符（Metric）千万别选错**：
* 如果数据集是 `angular`，`query.py` 必须传 `--metric cosine`。
* 如果选错了，算出来的召回率（Recall）会低得离谱，因为数据库在用“米”量距离，而数据集期望用“角度”量距离。


2. **为什么要有 `load.py` 里的 `batch_size`？**
* 就像搬砖，一块一块搬（单条插入）效率极低，一车一车搬（Batch）才能起量。实验中设置 1000 左右是性能平衡点。


3. **`EXPLAIN` 的妙用**：
* 运行测试前，先带 `--explain true --query_num 1` 看一眼。如果看到 `Index Scan`，说明索引生效了；如果看到 `Seq Scan`，说明索引没建对。
