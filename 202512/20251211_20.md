...
这张图展示了如何将传统的数据库表（关系模型）转变为图模型。

```mermaid
graph TD
    subgraph Relational_Tables [关系型数据库表]
        ST[Student表<br/>id, name]
        SC[School表<br/>id, name]
        KN[Know表<br/>src, dst]
        SA[StudyAt表<br/>pId, sId]
    end

    Relational_Tables -->|CREATE PROPERTY GRAPH| PG

    subgraph PG [属性图模型 Property Graph]
        direction LR
        P1((Person1)) -- Knows --> P2((Person2))
        P1 -- StudyAt --> SCH{School}
        P2 -- StudyAt --> SCH
    end
...
属性图最强大的功能是挖掘隐藏的关系。以下是几个核心挖掘特征的直观表示。

```mermaid
graph LR
    subgraph ShortestPath [1\. 最短路径]
        A((A)) -.-> B((B)) -.-> C((C))
        A == Shortest ==> C
    end

    subgraph NDegree [2\. N度关系]
        Me((我)) -- 1度 --> F1((朋友)) -- 2度 --> F2((朋友的朋友)) -- 3度 --> F3((陌生人))
    end

    subgraph CommonFriend [3\. 共同好友]
        U1((用户1)) --> MF((共同好友))
        U2((用户2)) --> MF
    end

    subgraph Community [4\. 连通分量/社交圈]
        subgraph Circle1
            N1(( )) --- N2(( )) --- N3(( ))
        end
        subgraph Circle2
...
这组图解释了 `SUMMARIZE` 命令背后的数学含义。

```mermaid
graph TD
    subgraph Connectivity [连通性分析]
        direction LR
        L1(( ))---L2(( ))---L3(( ))
        R1(( ))---R2(( ))
        Note1[连通分量=2<br/>孤立节点检测]
    end

    subgraph Clustering [聚集性分析]
        direction LR
        C1((我)) --- C2((朋友A))
        C1 --- C3((朋友B))
        C2 -. 闭环? .- C3
        Note2[聚集系数越高<br/>朋友圈越紧密]
    end
```
...
通过“走迷宫”的逻辑帮助学生记忆两种基础算法。

```mermaid
graph TD
    subgraph BFS [广度优先: 波纹扩散]
        direction TB
        B1((Start)) --> B2((1层)) & B3((1层))
        B2 --> B4((2层))
        B3 --> B5((2层))
    end
...
### 5. DuckDB-PGQ 性能优化：SIMD 与双向搜索

这张图展示了为什么 DuckDB 跑图搜索比传统 SQL 快。

```mermaid
graph TD
    subgraph SIMD_Parallel [SIMD 并行处理]
        direction LR
        Q[1个指令] --> L1[搜索任务1] & L2[搜索任务2] & L3[搜索任务3] & L4[任务...]
        Note[同时处理 64/512 个比特位]
    end

    subgraph Bidirectional [双向 BFS 优化]
        direction LR
        Src((起点)) --> S1(( )) --> S2(( ))
        Dst((终点)) --> D1(( )) --> D2(( ))
        S2 == "相遇" ==> D2
...
### 6. DuckDB-PGQ 优劣势分析 (学生选型参考)
  
| 特性 | DuckDB-PGQ (OLAP图) | 专门图数据库 (如 Neo4j) |
| --- | --- | --- |
| **优势** | **零数据迁移**（直接查关系表） | **极深深度遍历**（千万级跳数）|
| - | **极致分析性能**（向量化执行） | **高并发写入**（事务支持更好）|
| - | **SQL 标准**（易学易用） | - |
| **劣势** | 复杂递归写起来较繁琐 | 需要独立运维一套数据库 |
