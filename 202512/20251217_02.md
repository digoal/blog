...
PS: 高手也可以选择使用 vchordrq 向量索引或其他距离算子. 只要能满足 `limit 50` recall 大于等于 85%   
...
比赛的核心部分, 2、检查 recall 
```
-- 结合以上索引, 并调整以下语句中的 hnsw.ef_search = ?  
...
begin  
  ...
  raise notice 'recall(必须大于等于 85 %%), 实际值为: % %%', 100*v_recall;  
end  
$$ language plpgsql;  
  
-- 返回结果如下  
NOTICE:  recall(必须大于等于 85 %), 实际值为: 40 %
DO 
...
只有满足 recall 大于等于 85 % 之后, 可以开始进行压力测试    
  
比赛的核心部分, 3、压力测试  
```
-- 先回到 docker 容器的 shell 环境, 创建测试语句  
-- 将下面的 ef_search 修改为你的 hnsw.ef_search 最终配置(即满足 recall 大于等于 85 % 时的 hnsw.ef_search)  
...
不断优化, 将满足召回率的要求时 pgbench 的压测结果 tps 逼近最高值.   
```
-- 可修改: m=?, ef_construction=? , 修改这2个值需要删除并使用新的参数重新创建索引.    
-- 可修改: hnsw.ef_search = ?;   仅修改这个参数不需要重建索引. 
-- 可修改: pgbench -c ? -j ? , 修改这个参数不需要重建索引.  ( 最后一项, 增加一点趣味性, 因各自的电脑性能配置可能不同. )
```
