...
-- 可调整 shared_buffers 参数
-- 查看测试表的大小
select pg_size_pretty(pg_total_relation_size('vector_test'));
-- 查看 shared_buffers
show shared_buffers;
-- 可以修改 shared_buffers 至少大于测试表, 让数据都缓存到磁盘中, 可以避免磁盘性能问题带来的干扰
\! echo "shared_buffers='1GB'" >> /var/lib/postgresql/18/docker/postgresql.auto.conf
-- 然后在另一个cmd/powershell/shell中重启docker容器
docker stop pg
docker start pg
-- 然后重新进入容器、再进入数据库进行测试
docker exec -ti pg bash
