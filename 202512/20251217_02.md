...
```
比赛目的: 在 PostgreSQL 数据库中, 创建并优化向量索引参数( m=?, ef_construction=? ), 设置查询参数( set hnsw.ef_search = ? ; ). 在保证召回率(recall)高于 85% 的前提下, 尽可能的提高以下SQL查询的性能.    
...
-- 比赛过程  
-- 启动 docker engine , 然后启动并进入 pg 容器  
docker start pg  
...
-- 比赛的核心部分, 1、创建索引  
-- 请根据这几天的学习, 修改索引参数.   
-- 修改索引参数: m=16, ef_construction=64   
-- 例子  
...
declare  
  ...
  v_recall float8;  
begin  
  set local hnsw.ef_search = 20;  -- 当查询参数 = 20 时, recall 的值是多少?    
  set local enable_seqscan=off;  
  ...
  raise notice 'recall(必须大于0.85), 实际值为: %', v_recall;  
end  
$$ language plpgsql;  
  
-- 返回如下  
NOTICE:  recall(必须大于0.85), 实际值为: 0.4   
DO  
说明 recall=40% , 不符合要求, 需要调整索引参数或查询参数.  
-- 请修改: m=16, ef_construction=64 , 修改这2个值需要删除并使用新的参数重新创建索引.    
-- 请修改: set hnsw.ef_search = 20;   -- 设置查询参数: set hnsw.ef_search = 20;

  
   
-- 满足 recall 大于 0.85 时, 可开始进行压力测试  
   
    
-- 比赛的核心部分, 3、压力测试  
-- 先回到 docker 容器的shell环境, 创建测试语句  
-- 将下面的 ef_search 设置为你的 hnsw.ef_search 最终配置(即满足大于 0.85 时的 hnsw.ef_search)  
...
-- 不断优化, 将 pgbench 结果 tps 逼近最高值
-- 请修改: m=?, ef_construction=? , 修改这2个值需要删除并使用新的参数重新创建索引.    
-- 请修改: hnsw.ef_search = ?;   仅仅修改这个参数不需要重建索引 
```
