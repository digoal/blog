## 看不上AI的都会被AI抛弃: 人与AI和谐共生, 我1分钟搞定了向量教学例子  
...
## 背景   
最近发了一些文章, 总有人下面留言评论有AI味, AI味咋了? 用AI咋了? 用AI遭人嫌弃?   
  
**真的别看不上AI了, 我的态度: 所有人都能用AI提效, 但是能把AI发挥到极致的人, 本身肯定也是这个领域的半个专家.**    
    
因为下面的这个教学需要, 需生成一些符合正态分布的向量数据(不能纯随机, 否则就失去了现实意义, 例如群体的聚集性、语义相似排序的意义).     
- [《大学生数据库实践课开课了 : 1 大纲》](../202512/20251202_10.md)      
- [《大学生数据库实践课, docker 镜像》](../202512/20251205_02.md)      
- [《大学生数据库实践课: 课后大作业》](../202512/20251217_02.md)      
    
AI 一下就搞定了, 我只提供了话术, 以及后续的稍作修改, 搞出一个适合的版本. 并行的话就不劳烦函数了, pgbench 开启压测模式即可.    
  
(不过重点是你得懂业务啊, 你不懂业务怎么也不可能生成复合业务的数据, 至少得和AI高低多几轮对话, 走弯路对吧!)  
...
## AI 抢答    
在 PostgreSQL 中处理向量数据，通常推荐安装 **`pgvector`** 插件（目前已经是向量数据库的事实标准）。为了满足你“中心点聚合”和“概率分布”的需求，我们需要结合高斯分布（正态分布）的逻辑来编写函数。    
    
### 1\. 环境准备    
    
首先，确保你的数据库已经安装了 `pgvector` 插件。    
...
CREATE TABLE IF NOT EXISTS vector_test (    
    ...
    embedding vector(16),    
    is_center BOOLEAN -- 标记是否为中心点，方便后续查询    
);    
```    
...
我们需要利用 PostgreSQL 的内置函数 `random()`。为了实现“离中心点越近概率越高”，最科学的方法是使用**Box-Muller 变换**来生成符合 **正态分布（Normal Distribution）** 的随机数。    
    
#### 编写函数：`generate_vector_data`    
...
DECLARE    
    ...
    center_vec FLOAT8[];    
    new_vec FLOAT8[];    
    std_dev FLOAT8 := 0.1; -- 标准差，值越小点越密集，越大离散度越高    
    i INT;    
    j INT;    
    u1 FLOAT8;    
    u2 FLOAT8;    
...
```sql    
-- 生成 1000 个中心点，每个中心点生成 2 个周边点（共 1000 + 2000 = 3000 条数据）    
...
-- 查询距离某个中心点最近的 5 个点，看看是不是刚才生成的“追随点”    
...
### 💡 关键点解析    
  ...
  * **概率分布模型**：代码中使用了正态分布（高斯分布）。正态分布的特性是数值出现在均值（这里是中心点维度）附近的概率最大，越往外概率呈指数级下降，完美符合你“近处多、远处少”的需求。    
  * **Box-Muller 变换**：这是在计算机中通过两个均匀分布的随机数生成正态分布随机数的标准算法。    
    $$z = \sqrt{-2\ln(u_1)} \cos(2\pi u_2)$$    
  * **控制离散度**：变量 `std_dev`（标准差）控制了点的“紧凑程度”。    
      * 如果 `std_dev` = 0.01，点会非常贴近中心点。    
      * 如果 `std_dev` = 0.5，点会显得非常散乱。    
    
## 加餐, 标准差详解    
    
很多初学者对标准差的概念不理解，我们可以用一个 **“射击/打靶”** 的比喻让你瞬间秒懂：    
    
### 1. 标准差 ( $\sigma$ ) 是你的“准星精度”（尺子）    
想象你在打靶，靶心就是**平均值（Mean）** 。    
* **标准差**不是百分比，而是你偏移靶心的**平均物理距离**（比如 1 厘米、5 厘米）。    
* 它是一把**特制的尺子**。每个数据集的尺子长短不一样。有的班级考试分数波动大，它的“尺子”一格就是 20 分；有的波动小，一格就是 2 分。    
    
### 2. 那 68%、95%、99.7% 是什么？    
这些百分比是 **“覆盖率”** ，也就是你“子弹落在这个范围内”的**概率**。    
    
* **1 个标准差（ $1\sigma$ ）：** 相当于你拿着那把“尺子”，从靶心向左、向右各量出 **1 格**的距离。在这个圈圈里，你会发现你 **68%** 的子弹都落在里面了。    
* **2 个标准差（ $2\sigma$ ）：** 从靶心向左、向右各量出 **2 格**。这个圈变大了，你能圈住 **95%** 的子弹。    
* **3 个标准差（ $3\sigma$ ）：** 量出 **3 格**。这几乎覆盖了整个靶子，**99.7%** 的子弹都在这了。剩下的 0.3% 就是所谓的“奇葩”或“极端异常值”。    
    
### 3. 用前面提到的 16 维向量举例    
假设你生成向量时，某一个维度的平均值是 `0.5` (例子中使用`random()`函数生成的数据, 平均值就是0.5, 因为`random()`返回的是0到1之间随机分布的值.)，你设置的标准差 `std_dev` 是 `0.1`。    
    
* **这把尺子的 1 格 = 0.1。**    
* **68% 的概率：** 你生成的数值会落在 $[0.4, 0.6]$ 之间（ $0.5 \pm 0.1$ ）。    
* **95% 的概率：** 你生成的数值会落在 $[0.3, 0.7]$ 之间（ $0.5 \pm 0.2$ ）。    
* **99.7% 的概率：** 你生成的数值会落在 $[0.2, 0.8]$ 之间（ $0.5 \pm 0.3$ ）。    
...
| :--- | :--- | :--- |    
| **平均值 (Mean)** | 数据的中心点 | 靶心 |    
| **标准差 ( $\sigma$ )** | **单位长度** | 尺子上的“一格”有多长 |    
| **68% / 95%...** | **概率/面积** | 这个圈里能装下多少个数据点 |    
    
**所以，当你看到“3个标准差”时，把它想成“3倍距离”，而不是“3%”。**    
    
在工业界（比如“六西格玛”管理），追求的就是 $6\sigma$ 的精度，意味着 100 万个产品里只有 3.4 个次品，这就是把“圈子”拉得极大，让错误发生的概率变得极小。    
    
---    
    
## 人工修改    
我故意留了个维度/标准差可调, 总要东东过脑子, 否则全让AI嘎了, 一点意思都没有.    
...
CREATE OR REPLACE FUNCTION generate_vector_data(    
    ...
    points_per_center INT, -- 每个中心点周围生成的点数 (例如 2, 总计 2000)    
    dims int,  -- 维度    
    i_std_dev float8 default 0.1  -- 标准差，值越小点越密集，越大离散度越高    
) RETURNS TEXT AS $$    
DECLARE    
    center_id INT;    
    center_vec FLOAT8[];    
    new_vec FLOAT8[];    
    std_dev FLOAT8 := i_std_dev; -- 标准差，值越小点越密集，越大离散度越高    
    i INT;    
    j INT;    
    u1 FLOAT8;    
    u2 FLOAT8;    
...
pgbench 并行写入脚本 1.sql     
    
```sql    
SELECT generate_vector_data(:center_count, :points_per_center, :dims, :i_std_dev);    
```    
    
例如, 开10个并行, 每个负责300个中心点, 共3000个中心点. 每个中心点周围100条记录. 一共30万条记录.     
```bash    
...
## 再次升级需求    
````    
> 我要在PostgreSQL中创建一个存储向量的表, 写一个PostgreSQL函数, 生成测试数据, 需求如下:    
每个维度的取值区间为[0,1), 经过扰动得到的值也必须落在这个区间, 如果不能落在这个区间, 截取为上界或下界.    
16维(可变量输入).    
生成1000个(可变量输入)完全离散随机的向量.    
然后再以这1000个向量为中心, 生成2000个(可变量输入)离散向量.    
每个中心点周围数据的聚集性用标准差(通过下面的方法计算得出, 均值+扰动, 且必须为正值)来表示, 离中心点距离越近出现概率越高, 越远概率越低.     
每个中心点聚集起来的数据看作一类, 不同类的聚集性强弱也呈现正态分布. 代表聚集强弱的标准差的均值可变量输入(例如 0.1), 分布强弱可变量输入(例如0.01);    
...
1. **向量维度可变**（默认16）。    
2. **每个中心点完全离散随机**（取值 ∈ [0,1)）。    
3. **围绕每个中心点生成聚集点**：    
   - 使用正态分布扰动（均值=中心点值，标准差=某个类内标准差）。    
   - 扰动后值必须 ∈ [0,1)，否则截断到 [0,1)。    
4. **每个中心点的类内标准差本身服从正态分布**：    
   - 均值 = `std_dev_mean`（如 0.1）    
   - 标准差 = `std_dev_sigma`（如 0.01）    
   - 且标准差必须为正（截断到 > 0）。    
5. **支持 PostgreSQL `vector` 类型**（需安装 `pgvector` 扩展）。    
    
    
### ✅ 修改说明    
- 修改函数签名增加 `std_dev_mean` 和 `std_dev_sigma`。    
- 在每轮生成中心点时，**为该中心点生成独立的类内标准差** `local_std_dev`。    
- 对每个扰动后的维度值做 **[0,1) 截断**。    
- 使用 `GREATEST(0, LEAST(1 - 1e-9, value))` 确保严格在 [0,1)。    
- 提前检查 `vector` 类型是否存在（否则会报错）。    
...
```sql    
-- 确保已安装 pgvector 扩展（如未安装，请先执行：CREATE EXTENSION IF NOT EXISTS vector;）    
-- 表结构示例（请先创建）：    
    ...
    FOR i IN 1..center_count LOOP    
        ...
        FOR j IN 1..points_per_center LOOP    
            ...
            FOR k IN 1..dims LOOP    
                ...
                -- 截断到 [0, 1)    
                -- 使用 1 - 1e-9 避免等于 1（因 random() 本身 <1，但扰动可能 >=1）    
...
```sql    
-- 生成 1000 个中心点，每个中心生成 2 个邻居（共 3000 条记录）    
-- 16 维，类内标准差均值=0.1，波动=0.01    
...
2. **表结构需包含 `center_id` 字段**（用于追踪每个点属于哪个中心）：    
   ```sql    
   CREATE TABLE vector_test (    
       id SERIAL PRIMARY KEY,    
       embedding VECTOR,    
...
3. **性能提示**：生成 3000 条记录很快，但若数量极大（如 100 万），建议分批或使用更高效方法（如 `generate_series` + `CROSS JOIN LATERAL`），但当前函数结构清晰，适合中小规模测试数据。    
    
4. **截断处理**：使用 `1 - 1e-9` 确保最终值严格小于 1，符合 `[0,1)` 要求。    
...
效果如下, 符合预期, 有一些外围的点可能离其他中心点更近, 例如11明显离3057中心点的值更近.     
```sql    
SELECT * FROM vector_test     
ORDER BY embedding <-> (SELECT embedding FROM vector_test WHERE id = 11)     
LIMIT 10;    
...
# 附录: PolarDB 比赛用例    
如果打PolarDB比赛, 本文也非常适用.   
  
可采用如下结构和方法生成数据.      
    
使用了vector和vchord 2种插件, 可对比两种插件的效果.    
...
10个进程, 每个负责100个中心点, 每个中心点附近999条. 共100万条记录(比赛是1000万条, 只要改成`center_count=10000`):    
```bash    
...
接下来创建vchordrq、vector hnsw索引, 对比构建耗时和同等recall下的查询rt.    
...
```  
\timing  
set maintenance_work_mem='1GB'; -- 确保大于表, 确保全内存图构建  
...
with recall as (  
  select count(*) as cnt from hnsw_indexscan join hnsw_seqscan using (id)  
),   
seqscan as (  
 select count(*) as cnt from hnsw_seqscan  
)  
...
vchordrq 不支持unlogged table? 改用logged table.  
```  
set polar_force_unlogged_to_logged_table=off;    
...
```  
\timing  
set maintenance_work_mem='1GB'; -- 公平起见和vector设置一样, 实际上vchordrq不需要这么多内存.  
...
with recall as (  
  select count(*) as cnt from vchordrq_indexscan join vchordrq_seqscan using (id)  
),   
seqscan as (  
 select count(*) as cnt from vchordrq_seqscan  
)  
select recall.cnt/seqscan.cnt as recall from recall,seqscan;  
...
结论: 和vectorchord官方说的一样, 在保证召回率和查询性能的情况下, 相比hnsw构建索引的速度大幅度提升, 并且索引也更小.    
