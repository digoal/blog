...
set diskann.query_rescore = 20;  -- 如果索引使用了量化技术, 建议回表进行重排序 , 获得更好的 recall  
...
begin  
  ...
  set local diskann.query_rescore = 75;   -- 检查当查询参数 diskann.query_search_list_size = 50 时, 返回 top-50 的 recall 的值是多少?  
  set local enable_seqscan=off;  
  set local enable_indexscan=on;  
