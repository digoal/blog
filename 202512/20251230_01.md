...
- https://bailian.console.aliyun.com/?tab=api#/api/?type=model&url=2780056   
   
![pic](20251230_01_pic_013.png)  
  
reranker API 实验: 因 ollama 目前不支持 reranker API, Dify 中如果要增加 rerank 功能请使用云端大模型 API.  
  
**关于 reranker 错误的示范1:**  
  
Reranker 模型（如 `bge-reranker-v2-m3`、`Qwen3-Reranker-0.6B`）不是语言生成模型，它们的输出是一个 标量分数（float），表示 query 和 document 的相关性。  
  
但 Ollama 的 `/api/generate` 接口会：  
- 把输入当作文本 prompt  
- 让模型“继续生成”下一个 token  
- 返回的是 生成的文本（string），不是相关性分数  
...
{  
  "model": "...",  
  "response": "relevant",   // 或 "0.85" 字符串，甚至乱码  
  "done": true  
}  
```  
  
这根本不是真正的 reranker 分数，而是模型胡乱生成的 token！  
...
**正确姿势: 以上方法 vs 云端 Reranker 的本质区别**  
  
| 维度 | 你的方法（Embedding + Cosine 距离） | 云端 Reranker（如 bge-reranker-v2-m3） |
|------|----------------------------------|-------------------------------------|
| **任务类型** | 双塔结构：query 和 doc 独立编码 → 计算相似度 | 交叉编码（Cross-Encoder）：将 query + doc 拼接后整体理解 |
| **语义深度** | 浅层语义匹配，无法捕捉细粒度交互（如否定、条件、因果） | 深度语义交互，能判断“是否真正回答了问题” |
| **模型能力** | qwen3-embedding:0.6b 是轻量级 embedding 模型，训练目标为 retrieval，非 reranking | 专用 reranker 在大量标注 pair（相关/不相关）上微调 |
| **排序质量** | 对主题相关性尚可，但对“是否精准回答 query”判断弱 | 能区分“相关但不直接” vs “高度相关且直接” |
| **计算开销** | 低（可预计算 doc 向量） | 高（每次需实时推理 query+doc 对） |
   
**所以目前要reranking, 必须用云端API**  
  
如果一定要在本地进行reranker, 不要通过 Ollama 调用 reranker，改用原生 Hugging Face Transformers. 这是最可靠的方式。示例代码（Python）：  
```python  
...
query = "如何通过饮食调节来降低高血压？"  
documents = [  
    "针对高血压患者，建议采用 DASH 饮食方案，即减少钠盐摄入，多吃富含钾、钙、镁的蔬菜水果和全谷物，这被证明能有效降低血压。",  
    "减少食盐摄入是控制血压的关键，每日摄入量应控制在 5 克以下，同时应避免食用腌制食品。",  
    "高血压是一种慢性病，除了药物治疗外，规律的体育锻炼、戒烟限酒以及保持心理平衡对于康复也至关重要。",  
    "低血糖患者在饮食上应注意少食多餐，适量补充糖分，避免空腹剧烈运动，以防止晕厥发生。"  
]  
...
with torch.no_grad():  
    ...
    print(scores.tolist())  # 输出浮点分数，如 [8.5, 6.2, 2.1, -3.0]  
```  
...
HF_ENDPOINT=https://hf-mirror.com python3 reranker.py  
```  
  
## Navicat 图形化数据库开发管理工具 & AI 功能体验
PS: 高校师生使用 Navicat 可申请 Navicat 教育版, 申请页面: https://www.navicat.com.cn/sponsorship/education  
...
-- 创建数据库 test  
create database test owner digoal;  
```  
...
![pic](20251230_01_pic_002.jpg)  
  
5、配置 Navicat AI 助手 (使用qwen3:4b)  
  
打开 Navicat 图形界面, 菜单栏选择设置.  
  
![pic](20251230_01_pic_003.jpg)  
  
配置 AI 助手  
  
![pic](20251230_01_pic_004.jpg)  
...
输入提示词
“我使用的是PostgreSQL, 创建一个测试表, 三个字段: ID, info, ts. 往表里写入100万行随机数据.”

发送提示词

输入提示词
“查询这个表的总记录数, 采样查询这张表的随机10条记录.”

发送提示词
```  
  
![pic](20251230_01_pic_005.jpg)  
  
拷贝到面板中执行  
  
![pic](20251230_01_pic_006.jpg)  

使用 AI 助手修复报错  
  
![pic](20251230_01_pic_007.jpg)  
  
**其他Navicat功能请自行探索, 例如BI数据挖掘和可视化, 数据建模, 调试函数, 数据编辑等等**  

## AI Coding Agent 实验
参考: [《大学生数据库实践课: 7.5 AI Coding Agent 的使用》](../202601/20260105_01.md)
...
## dify  
下载 dify 相关容器, 启动 dify, 参考: [《大学生数据库实践课: 10 Dify RAG 配置详解》](../202512/20251210_19.md)  
  
配置 dify 参考 [《3分钟速玩Dify(高度可定制的企业级AI应用开源项目)》](../202504/20250404_01.md)  
...
使用pgbench , 10个进程, 每个负责100个中心点, 共1000个中心点. 每个中心点附近根据高斯分布写入999条记录. 32个维度. 共100万条记录 :  
```sql  
-- 插入100万条测试数据  
-- category: 0-9  
-- price: 0-1000  
...
qwen AI:   
使用 pgvector 时, 100万条记录, 将ivfflat lists从1000改到100, probes=1不变, recall 反而下降. 我怀疑是采样数也随之下降, 导致区域中心点选择不佳. 请先判断我的猜测是否准确, 增加采样数固定的功能, 创建 ivfflat 索引时通过一个参数来输入采样条数.   
- https://github.com/pgvector/pgvector/pull/946  
  
#### 5.1 用 pgadmin 观察泰森多边形(思考和ivfflat索引中心点及聚集区域的特征相似性)    
1、安装 pgadmin  
...
![pic](20251230_01_pic_008.jpg)  
  
3、创建 postgis 插件    
  
打开 query 界面, 创建 postgis 插件  
  
![pic](20251230_01_pic_009.jpg)  
  
```  
create extension if not exists postgis;  
```  
  
![pic](20251230_01_pic_010.jpg)  
...
7、在 pgadmin 中观察泰森多边形     
  
```  
select * from tb;  
```  
  
![pic](20251230_01_pic_011.jpg)  
  
![pic](20251230_01_pic_012.jpg)  
  
    
思考: ivfflat 向量索引的中心点具有泰森多边形中心点一样的什么特性?     
...
- [《最近邻索引 (Nearest Neighbor Indexes)：Pgvector 中的 IVFFlat 索引是什么，它们如何工作》](../202512/20251202_09.md)    
...
-- 查询模型表 model_model1 (也可以理解为documents对应的词典表) , 现在还没有往表里写入数据, 所以下面的 query 应该没有记录
select * from model_model1;
-- 现在应该是空的, 因为没有往documents表里面写记录, 所以也没有词典
...
select * from model_model1;  -- 现在词典里面肯定有数据了.

-- 现在查询一下 documents 这张表, 看看是不是有 这些编号?
...
-- 调整 bm25 得分排名第一的句子, 加一些无关的内容.  让这句话的长度(准确的说是token数)超过第二句话
...
`recall = 0%` 在真实环境是是非正常现象, 因为我们这个模拟数据(每999条)与中心点太靠近了, 所以量化之后的值没有了差异化, 所以通过量化相似返回的结果大量都是一样的值, 因此`recall=0`.  
...
-- 确认 标量条件 和 全文检索 条件都已经使用了最佳索引

-- 联合索引
-- "idx_vector_test_category_price" btree (category, price)
-- 全文检索索引
-- "idx_vector_test_tsvec" gin (tsvec)
```
...
下面的实验要从网络上下载并绑定测试数据集. 如果无法完成下载, 请先从外部能下载的机器进行下载(例如拷贝到 `c:/` ), 然后拷贝到 pg 容器在宿主机上的卷目录中( 在cmd中执行 `docker cp c:/snb.duckdb pg:/var/lib/postgresql/` ), 进入容器后, 直接在duckdb cli中修改文件权限例如 `.shell chmod 777 snb.duckdb` . 并将 ATTACH 命令改成本地路径, 例如 `ATTACH '/var/lib/postgresql/snb.duckdb';`  
...
WHERE src_Id <> dst_id;
-- 用通俗的话说，它在查询：
-- “有哪些已封禁的黑名单账户，曾经和我的目标账户（src）通过同一个中间人产生过资金往来？”  
-- 应用场景： 如果一个账户（mid）既给“张三”转过钱，也给“电信诈骗嫌疑人（dst）”转过钱，那么“张三”也可能存在欺诈风险或洗钱嫌疑。 
...
-- 检测循环担保路径（使用 ANY SHORTEST）  
SELECT DISTINCT   
    company_id,  
    cycle_length  
FROM GRAPH_TABLE(finbench  
    MATCH p = ANY SHORTEST (c1:Company)-[g1:companyGuarantee]->+(c2:Company)  
    WHERE c1.companyId = c2.companyId  
    COLUMNS (c1.companyId as company_id, path_length(p) as cycle_length)  
) circular_guarantees  
ORDER BY company_id, cycle_length;
```
  
检测通过共同媒介关联的欺诈团伙:   
  
这个图展示了 SQL 中 MATCH 子句定义的实体关系：一个媒介连接两个账户，且这两个账户之间有资金往来。  
```mermaid
graph TD
    M[Medium: 登录媒介] -- s1:signIn --> A1(Account 1: 账户A)
    M -- s2:signIn --> A2(Account 2: 账户B)
    A1 -- t1:transfer --> A2
...
这个图展示了经过 GROUP BY 和 HAVING 过滤后的最终形态：一个媒介下聚集了多个（≥3个）互有往来的异常账户。  
```mermaid
graph LR
    subgraph Fraud_Group ["欺诈团伙 (shared_medium)"]
        M((Medium))
        A1[Account A]
        A2[Account B]
        A3[Account C]
    ...
    style Fraud_Group fill:#fff0f0,stroke:#ff0000,stroke-dasharray: 5 5
    style M fill:#f9f
```
...
```mermaid
stateDiagram-v2
    [*] --> Graph_Match: 匹配 Medium-Account-Transfer 模式
    Graph_Match --> Filter: WHERE a1 <> a2 (排除自转账)
    Filter --> Columns: 提取 ID (mediumId, accountId, etc.)
    Columns --> GroupBy: 按 shared_medium 聚合
    GroupBy --> Having: 过滤账户数 < 3 的组
    Having --> OrderBy: 按关联数降序排列
    OrderBy --> [*]: 输出报告
```
...
SELECT DISTINCT   
    ...
    ARRAY_AGG(DISTINCT account_id) as account_list,  
    COUNT(DISTINCT from_id) as active_transfers  
FROM GRAPH_TABLE(finbench  
    MATCH (m:Medium)-[s1:signIn]->(a1:Account),  
          (m:Medium)-[s2:signIn]->(a2:Account),  
          (a1:Account)-[t1:transfer]->(a2:Account)  
    WHERE a1.accountId <> a2.accountId  
    COLUMNS (  
        m.mediumId as shared_medium,  
        a1.accountId as account_id,  
        a2.accountId as account2_id,  
        t1.fromId as from_id  
    )  
) medium_connections  
GROUP BY shared_medium  
HAVING COUNT(DISTINCT account_id) >= 3  
ORDER BY connected_accounts DESC, active_transfers DESC;
```
   
下面是去除了账号往来信息的SQL, 仅仅找出有多个账号的共同媒介  
```sql
-- 快速识别高风险媒介  
SELECT   
    shared_medium,  
...
这个图展示了 SQL 中 MATCH 子句定义的 **“星型”** 关联模式：一个媒介（Medium）作为中心枢纽，同时连接了两个不同的账户。
```mermaid
graph TD
    M[Medium: 登录媒介] --- s1((signIn)) --- A1(Account 1)
    M --- s2((signIn)) --- A2(Account 2)

    style M fill:#f9f,stroke:#333,stroke-width:2px
    style A1 fill:#dcf,stroke:#333
    style A2 fill:#dcf,stroke:#333
```
  
高风险媒介聚集图 (`HAVING ≥ 3`) 当查询结果输出时，它代表了如下的结构。一个媒介下挂载了多个账户，形成了一个明显的风险簇（Cluster）。  
```mermaid
graph LR
    M{{"Medium<br/>(shared_medium)"}}
    ...
    style Risk_Cluster fill:#fff5f5,stroke:#ff4d4f,stroke-dasharray: 5 5
```
  
数据处理流向图, 展示了数据从属性图原始数据到最终排序列表的演变过程：
```mermaid
stateDiagram-v2
    direction LR
    state "Graph Matching" as match
    state "Filter: a1 <> a2" as filter
    state "Group By Medium" as group
    state "Count Accounts ≥ 3" as having
    state "Order by Density" as sort

    [*] --> match: 匹配共用媒介模式
    match --> filter: 剔除自匹配
    filter --> group: 按媒介 ID 聚合账户
    group --> having: 识别“多开”行为
    having --> sort: 优先排查最密集媒介
    sort --> [*]: 输出高风险名单
```
