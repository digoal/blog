...
索引通过自定义物理计划节点与 DataFusion 查询执行管道集成，这些节点执行索引查找并将结果转换为用于数据获取的行地址。有关全面的查询执行细节，请参见 查询执行管道 (4.2)。  
...
Lance 通过 `optimize_indices()` 操作支持增量索引更新，该操作合并多个索引版本并包含未索引的数据。参见 索引管理 (7.3) 了解详细的优化策略。  
