...
<figure>
    ...
    <figcaption>图 1：IOT 的关键不是“多建一个索引”，而是把表本体放进主键 B+Tree。主键点查和范围扫描减少一次 heap 跳转，但写入、页分裂、二级索引和大行处理会变得更敏感。</figcaption>
  </figure>
...
<figure>
    ...
    <figcaption>图 2：IOT 的主路径很短，但宽行、二级索引和主键变化会把复杂度带回来。Oracle IOT 论文专门讨论了 overflow 与 secondary index；OrioleDB 则在 PostgreSQL 表访问方法下重新实现主存储和索引行为。</figcaption>
  </figure>
...
<figure>
    ...
    <figcaption>图 3：IOT 将主键读优化到主路径，但写入会直接作用于主键树。顺序插入通常友好；随机插入、宽行更新和主键更新会放大页分裂、重平衡和二级索引维护。</figcaption>
  </figure>
...
<figure>
    ...
    <figcaption>图 4：undo-based MVCC 的价值在于把历史版本从主键树主路径中拿出去。这样可以保留 IOT 主路径的紧凑性，但代价是 undo 生命周期、快照保留、崩溃恢复都更复杂。</figcaption>
  </figure>
...
<figure>
    ...
    <figcaption>图 5：IOT 是强假设技术。越能确认主路径、主键稳定性、行宽和二级索引边界，越容易获得收益；反之，heap 的通用性往往更稳。</figcaption>
  </figure>
