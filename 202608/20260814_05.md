...
- [一、迁移方法论总览（KCSM 五步法 + 决策启发式）](#一迁移方法论总览)
- [二、测试环境搭建（Oracle + KES 双容器）](#二测试环境搭建)
- [三、E1 评估阶段（KDMS 4 象限兼容性扫描）](#三e1-评估阶段kdms-4-象限)
- [四、E2 改造阶段（SQL/PLSQL 方言适配）](#四e2-改造阶段方言适配)
- [五、造数据（业务 Schema + 测试数据）](#五造数据业务-schema--测试数据)
- [六、E3 测试阶段（KDTS 全量 + KFS 同步）](#六e3-测试阶段kdts-全量--kfs-同步)
- [七、E4 割接阶段（业务不停机切换）](#七e4-割接阶段业务不停机)
- [八、E5 回退阶段（30 秒切回源库）](#八e5-回退阶段30-秒切回源库)
- [九、性能验证（KWR/KSH/KDDM 三件套）](#九性能验证kwrkshkddm-三件套)
- [十、附录（方言对照表 / JDBC / 快速验证命令）](#十附录)
...
| E3 测试 | KDTS + KFS | 全量迁移 + 同步验证 | 目标库 + KFS 比对报告（精简/详细两种模式） |
...
│                                                                             │
│  ┌─────────────────┐  代理/直连  ┌─────────────────┐  直连  ┌────────────┐  │
│  │ Oracle 23ai Free │ ─────────► │  KFS 源端服务    │ ─────► │ KES V9R2C14│  │
│  │  (Docker)        │            │  (Linux 容器)    │        │  (Docker)  │  │
│  │  端口 1521       │            │  端口 3112/11000 │        │  端口 54321│  │
│  └─────────────────┘            │  • Extract       │        └────────────┘  │
│                                  │  • Transfer      │              ▲         │
│                                  │  • Replicate     │              │         │
│                                  │  • Manager:8090  │              │         │
...
│                                          └──────────────────────────┘         │
│                                                                             │
│  阶段： E1评估 ─► E2改造 ─► E3测试(E3-KDTS+E3-KFS+E3-比对) ─► E4割接 ─► E5回退 │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
```mermaid
flowchart LR
    subgraph SRC["Oracle 源端 (Docker)"]
        ORACLE[(Oracle 23ai Free<br/>1521)]
    end

    subgraph PROXY["KFS 源端服务 (Linux 容器)"]
        EXTRACT[KFS Extract<br/>支持 Logminer/REDO]
        TRANSFER[Transfer<br/>KUFL 3112]
        REPLICATE[Replicate]
        MGR[Manager 8090<br/>管控平台]
        COMPARE[比对服务 8091<br/>精简/详细]
    end

    subgraph DST["KES 目标端 (Docker)"]
        KES[(KES V9R2C14<br/>54321)]
    end

    APP[业务应用 JDBC]

    ORACLE -- "本例: Logminer" --> EXTRACT
    ORACLE -. "备选: REDO 模式
需 KFS 与 Oracle 同机" .-> EXTRACT
    ORACLE == "推荐生产: REDO + 日志代理
(无侵入, 详见 §6.1)" ==> EXTRACT
    EXTRACT --> TRANSFER
    TRANSFER --> REPLICATE
    REPLICATE -- "DML/DDL" --> KES
    MGR -.- EXTRACT
    MGR -.- REPLICATE
    COMPARE -. "数据校验" .-> ORACLE
    COMPARE -. "数据校验" .-> KES

    APP -- "迁移期：JDBC oracle" --> ORACLE
    APP -. "T-10min 切换" .-> KES
    KES -. "应急回退<br/>(反向 KFS)" .-> ORACLE

    classDef stage fill:#e1f5ff,stroke:#0288d1,color:#000;
    classDef ops fill:#fff3e0,stroke:#f57c00,color:#000;
    classDef app fill:#f3e5f5,stroke:#7b1fa2,color:#000;
    class ORACLE,KES stage
    class EXTRACT,TRANSFER,REPLICATE,MGR,COMPARE ops
    class APP app
```
...
| 术语 | 含义 | 备注 |
|:---|:---|:---|
| 正向 KFS 链路 | Oracle → KES 的 KFS 同步服务（服务名 `ORA_TO_KES_SYNC`） | E3-E4 阶段的主链路 |
| 反向 KFS 链路 | KES → Oracle 的 KFS 同步服务（服务名 `KES_TO_ORA_REVERSE`） | 割接前预置但保持 offline，仅在 E5 应急回退时短暂启用 |
| 应用单切 | 割接窗口内一次性把业务 JDBC 从 Oracle 切到 KES，Oracle 端停写 | 配合正向 KFS 链路 drained 校验使用 |
| KFS 比对服务 | KFS 自带的数据一致性校验组件（精简/详细两种模式） | E3-E5 阶段产出"比对报告"，独立于 KDMS |
| KDMS 与数据校验 | KDMS 核心是结构迁移 + 4 象限兼容性评估，不提供两端数据比对 | 数据校验统一使用 KFS 比对服务 |
...
| 象限 | 兼容性 | 高风险项 |
|:---|:---|:---|
| 对象兼容 | 95%+ | `(+)` 外连接 → 改 ANSI JOIN；DBMS_JOB → 改 KES 调度（KDMS 翻译率覆盖不到的对象类型） |
...
|:---|:---|:---|
| `WHERE a(+)=b` | `WHERE a LEFT JOIN b` | 必须改 ANSI JOIN |
...
| 差异点 | Oracle | KES | 影响 |
|:---|:---|:---|:---|
| 空串 `''` | 等于 NULL | 不等于 NULL | 字符串判断逻辑可能反转 |
| 隐式类型转换 | 宽松 | 严格（Oracle 兼容模式） | `WHERE int_col = '123'` 在 Oracle 通过，在 KES 报错；如需宽松行为需显式设置兼容参数（生产环境慎用） |
...
| REDO + 日志代理（官方推荐的无侵入方案） | 开归档 + 开补充日志 + 单独的代理服务机器 | Oracle 端仅部署轻量日志读取代理；解析在外部"体外机"完成；省 Oracle 资源、保留 REDO 全部优点 |
...
> 💡 金仓 KFS 无侵入增量数据同步方案（推荐生产环境采用该方案）
>  
> 如果生产 Oracle 机器资源紧张、无法与 KFS 同机部署，但又需要 REDO 模式的解析能力（大对象/DDL 支持），可采用金仓的 无侵入方案：  
>  
> - 在 Oracle 端仅部署轻量化的增量日志读取代理服务（导管），只负责实时捕获归档日志，CPU/内存占用极低  
> - 抓取的日志通过专线实时传输到 KFS 源端"体外机" 上，由体外机完成解析、缓存、排序等重活  
> - 实际效果：Oracle 端资源占用 ≈ 1% CPU / 500MB 内存，传统集中部署要 10% CPU / 2GB 内存  
> - 本质上把 REDO 模式"同机部署"的硬约束变成了"代理 + 远程传输"，省下 Oracle 容器资源，又不损失 REDO 优点  
> - 适用：核心业务系统、对生产库稳定性要求极高、Oracle 资源紧缺的场景  
>  
> 本文后续默认仍以 Logminer 模式为主线（不破坏 Oracle 容器边界），生产环境强烈建议优先评估无侵入方案——尤其涉及大对象或 DDL 同步时。  
...
│  │  CPU≈1% / 500MB   │                                        │
│  └────────┬─────────┘                                        │
│           │ 轻量日志读取代理（仅占 CPU/内存 ≈ 1%）            │
│           ▼                                                    │
│  ┌──────────────────┐                                        │
│  │  日志代理服务     │ ────┐                                  │
│  │  (金仓 KFS        │     │ 实时专线推送归档日志              │
│  │   log-agent)     │     │ (rsync/专线/KSync)               │
│  └──────────────────┘     │                                  │
└───────────────────────────┼──────────────────────────────────┘
                            │
                            ▼
...
```mermaid
flowchart LR
    subgraph ORACLE_HOST["Oracle 主机（生产）"]
        ORACLE[(Oracle 23ai Free<br/>CPU 1% / 500MB)]
        AGENT["日志代理服务<br/>轻量, 仅转发归档"]
    end

    subgraph KFS_HOST["KFS 源端 体外机（独立）"]
        EXTRACT_REDO[KFS Extract<br/>REDO 解析<br/>重活在此]
        TRANSFER[Transfer<br/>KUFL 3112]
        REPLICATE[Replicate]
        KES[(KES V9R2C14<br/>54321)]
    end
    ...
    ORACLE -- "归档日志" --> AGENT
    AGENT -- "专线推送" --> EXTRACT_REDO
    EXTRACT_REDO --> TRANSFER
    TRANSFER --> REPLICATE
    REPLICATE --> KES

    classDef prod fill:#e1f5ff,stroke:#0288d1,color:#000;
    classDef agent fill:#fff3e0,stroke:#f57c00,color:#000;
    classDef kfs fill:#e8f5e9,stroke:#388e3c,color:#000;
    class ORACLE,KES prod
    class AGENT agent
    class EXTRACT_REDO,TRANSFER,REPLICATE kfs
```
...
│                                                                  │
│  ┌────────────────────────┐    ┌──────────────────────────┐     │
│  │   KFS Manager (8090)   │    │   KFS Replicate          │     │
│  │   管理控制台            │    │   同步引擎                │     │
│  │                        │    │                          │     │
│  │   - 节点管理            │    │   ┌──────────────────┐  │     │
│  │   - 任务配置            │    │   │ Extract 进程     │  │     │
...
| 模式 | 适用 | 优缺点 |
|:---|:---|:---|
| 精简校验（记录数 + 校验和） | 大表、校验时间紧 | 只校验行数和聚合校验和，速度快；不能识别字段级差异 |
| 详细校验（逐行逐列） | 核心表、数据准确性要求高 | 识别字段级差异，可推平；耗 CPU/IO，大表慎用 |
...
| 5 | KES 数据完整 | KFS 比对服务（或人工记录数比对） | 100% 一致 |
| 6 | KES 性能基线 | `sys_stat_statements` TOP 10 | 已采集 |
| 7 | 反向 KFS 链路（KES_TO_ORA_REVERSE）就绪 | `fsrepctl -service KES_TO_ORA_REVERSE status`（应保持 offline） | 30 秒可切回 |
...
| `NUMBER(p,s)` | `NUMERIC(p,s)`（存储类型）/ 应用层 `NUMBER` 写法照旧 | Oracle 兼容模式下 `CREATE TABLE ... NUMBER(p,s)` 语法兼容；底层映射到 NUMERIC（详见《KFS 数据类型映射参考手册》） |
