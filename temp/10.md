...
4、方法7，CTID合并扫描，与PostgreSQL 多索引的bitmapAnd, bitmapOr扫描类似，只是细化到行这个层级。

可以将多个索引扫描得到的CTID进行合并。跳过不需要扫描的行号。

如果

5、以无法为有法，以无限为有限，此乃武术最高境界。在PostgreSQL中，实现了多索引的bitmapAnd, bitmapOr扫描，极大的提高了多个条件（索引）带来的数据命中率。

[《PostgreSQL bitmapAnd, bitmapOr, bitmap index scan, bitmap heap scan》](../201702/20170221_02.md)  


## 小结
让索引更加精准，空间+时间+对象的分区索引
...
不过数据库会根据统计信息，自动判断是否使用单一索引，还是组合索引+BITMAP扫描。
