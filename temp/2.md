...
### 标签                                                                                                                                                          
PostgreSQL , OLAP , 向量化 , vector , postgrespro , tiles , 瓦片 , 瓦片索引 , map , reduce , 分组聚合 , 非分组聚合 , 分区键 , sort key , order by , brin , cpu Cache           
...
那么有没有更友好的方法呢，不修改PostgreSQL内核，不动它的架构：
...
<tr><td><code>betwixt(x,low,high)</code></td><td>Analog of BETWEEN</td></tr>
...
It can be done using unnest function:
...
Then it defines implicit type cast from vops_bool (result of boolean operation with tiles)
to boolean type. Now programmer do not have to wrap vectorized boolean operations in filter()
...
There is only one difference left between standard SQL and its vectorized extension. You still have to perform explicit type cast in case of using string literal, for example l_shipdate <= '1998-12-01'
will not work for l_shipdate column with tile type. Postgres have two overloaded versions
...
And it decides that it is better to convert string to the tile type vops_date.
...
(vector OP vector) operation instead of more efficient (vector OP scalar).



There is one pitfall with post parse analyze hook: it is initialized in the extension _PG_init function.
But if extension was not registered in shared_preload_libraries list, then it will be loaded on demand when any function of this extension is requested.
Unfortunately it happens after parse analyze is done. So first time you execute VOPS query, it will not be transformed. You can get wrong result in this case.
Either take it in account, either add vops to shared_preload_libraries configuration string.
VOPS extension provides special function vops_initialize() which can be invoked to force initialization of VOPS extension.
